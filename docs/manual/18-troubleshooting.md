...
| `context_overflow` | llm-client / 各算子 | 普通流提交前遇完整请求超限，会沿完整成员边界缩短冻结序列并重算会话下游。必要单帧、帧对、fixed Schema/指令或最小 pair 仍超限时按所属阶段终态失败；不会抽图、裁正文或把 pairwise 超限折算平局。预检与成功 HTTP 返回的超限不补喂熔断；reactive HTTP 400 的最小终态只喂一次。普通单记录保持原有预算与错误规则。 |
