...
probe llm.judge: FAIL HTTP 401: {"error":{"message":"token expired or incorrect",...}}
```
...
```
probe <kind>.<profile>: ok model=... latency_ms=...
probe <kind>.<profile>[<ENV名>]: ok model=... latency_ms=...
probe <kind>.<profile>[<ENV名>]: FAIL <错误信息>
```
