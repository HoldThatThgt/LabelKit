...
### 序列标注（v1.8）：episode 的「当前记录」段长什么样

开启分段算子（第 25 章）后，被标注的对象是整个 episode，「当前记录」段换成三段式呈现：

1. **`[动作序列]`**——extract 摘出的动作逐步列出（`1. click（对象: 搜索框）…`），给模型一条「发生了什么」的主线（extract 未开启时整段省略）；
2. **`[关键帧 i/k·成员 m]`**——挑出的关键帧逐帧「一段说明文字 + 一张截图」，给主线配上视觉证据（标签里的成员序数让模型能把图对回序列位置）；
3. **`[成员帧摘要]`**——收尾一段**全体**成员的逐帧文字摘要（总量封顶），保证没被选为关键帧的部分也不缺席。

关键帧数量由 `annotate.sequence_frames` 控制（默认 20，合法范围 **[2, 100]**）：成员数超过它时按确定性等距降采样选帧，首帧与末帧恒被保留。把它调到 **20 以上要当心一个供应商硬约束**：Anthropic 端点对「单请求超过 20 张图」的请求，任何一张图长边超过 2000px 就直接 400 **硬拒**（不是自动缩小）——而 `max_image_px` 默认恰是 2048。所以 M1 在「`sequence_frames > 20` 且所引 profile `max_image_px > 2000`」时会打警告：要么把 `max_image_px` 降到 ≤ 2000，要么降帧数。
