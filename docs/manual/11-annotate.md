...
普通流标注的对象是冻结后的完整序列视图。提示词保留动作步骤，并按成员顺序提供每个成员的完整文本、
完整可见规范化 UI 树和需要的截图。没有关键帧抽样、摘要字符上限或丢弃中间步骤的预算策略。
`annotate.sequence_frames` 已删除；普通流不使用 `input.ui_tree_max_chars` 裁剪成员树。
图片仍按部署声明的固定工作点发送，完整证据不代表无限上下文。

已知完整请求在分段/缝合候选形成时预检；标注结果、后处理和修复意见的长度只有运行中才能确定。
实际 ContextOverflowError 在正式提交前沿完整成员边界缩短序列，封闭前段并保留后段为后续序列，
重新执行未提交会话的下游。上游分段和缝合不重跑，已有容量边界不会重新并回去。
单个最小成员、固定指令或 Schema 仍超限时按标注阶段终态失败；超限不等于成功归档。

完整示例与验证入口见 [会话容量示例](../../examples/sequence-context-capacity/README.md)。
...
3. **UI 模态看图**——普通流先确认完整成员证据和出现位置；普通单记录再检查图片工作点是不是缩太狠导致小字不可读（v1.11 起日常发送尺寸由 `default_image_px` 决定，0 = 沿用 `max_image_px`，第 6 章）；树是不是被 `ui_tree_max_chars` 或预算份额截断了关键节点（trace 的序列化文本里找 `…(truncated`；声明预算的运行另可读 `report.budget.truncations` 的逐算子裁剪计数）；
