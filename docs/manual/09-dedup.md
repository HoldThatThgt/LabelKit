...
开启后对静态参与语义层的记录请求 embedding，再按输入序合并 exact、MinHash、pHash、semantic 判决。普通流发送完整规范化成员文本/UI 树，embedding profile 必须有正值 context_window；预检或实际超限交会话协调器拆分，致命 provider 错误不视为成功去重。普通单记录保留原有头部预算裁剪和 embedding 失败处理。UI image-only 的非序列记录只在图像解码失败时参与语义层。
...
v1.9 再开缝合算子（第 26 章）时规则不变、单元升级：判重单元是**线索**（thread）——指纹由重绑后的全部成员帧按序拼接而成（跨碎片），链序上缝合先于去重，判重面看到的就是最终线索；被并进线索的 `stitched` 壳已不是 `active` 状态，天然不参与判重，不会与幸存线索自撞。

普通流的本会话去重结果先留在内存增量中。若下游容量不足，整次尝试的增量丢弃，确定性拆分后重新判重；正式索引和先前会话结果保持原值。最终尝试完成时，一次提交 dedup 阶段接纳的全部身份。后续质量或评审淘汰不会追溯取消已接纳身份，因此跨会话重复判定继续遵循原有链序语义。
...
- `"batch"`：普通记录只查本批；普通流只查当前完整会话，计算组变化不重置索引。跨会话重复不会被这种作用域检出。
