...
| `batch_size` | int | 256 | 普通记录：批大小与 pairwise 比较池。普通流：单个计算组的叶任务上限；完整会话可以跨多个计算组，输出序列可大于此值。它不按帧数切会话，也不控制 prompt 预算或会话质量池。 |
...
| `ui_tree_max_chars` | 30000 | 普通单记录的控件树渲染上限。普通流使用每个成员的完整可见规范化 UI 树，不因该值或预算裁剪树；是否需要容量拆分按完整请求判断。 |
