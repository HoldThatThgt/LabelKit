...
| `[generate.stream]` | 关（v1.13，仅 `generate_only`） | `sessions`（会话数，交叉会话数 = Σsequences − sessions）、`noise_ratio`（噪音帧占比）、`duplicates`（原样重发条数）；子表 `[[generate.stream.tiers]]`（v1.14 帧类构成档位：`tier_rank` / `weight` / `frame_classes`），序列类可用同结构的 `[[class.<名>.generate.tiers]]` 整表覆盖它（v1.15） | 第 27 章 |
