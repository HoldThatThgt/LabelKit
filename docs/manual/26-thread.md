...
> 线索产物的账怎么对？**本章样例全部来自 `examples/stream`（UI 流工程 `project.toml`，
> 与第 25 章同一工程）的真实运行。
...
四类规范场景（也是 `examples/stream` 的 fixture 蓝本）：

| 场景 | 布局 | 预期标定 |
|---|---|---|
| 串联 | 前 x 帧任务 A，后 y 帧任务 B | 2 线索，零缝合 |
| 单交叉 | A 前半 → B → A 后半 | A = 双碎片线索（1 接缝），B = 单碎片 |
| 多交叉 | A₁ → B → A₂ → D → A₃ | A = 三碎片线索（2 接缝），B/D 各单碎片 |
| 噪声 + 救援 | A → B → 1 噪声帧 → A 的超短收尾段 | 噪声帧照旧剔除；短段命中救援并回 A |
...
## 26.2 快速上手：examples/stream 全流程

仓库自带的 `examples/stream`（`project.toml`）把四个规范场景加负样本装进一个工程：53 对帧、五个场景子目录，`[stream] key = ["source_dir"]` 让**每个子目录成为一个会话**（编号全树错开，互不冲突）：

- `s1-serial-noise/`（1–14）：点外卖 8 帧（帧 5 社交消息屏，预期噪声）+ 打车 5 帧背靠背 + 帧 14 桌面屏——串联 + 噪声，什么都不该缝（帧 14 预期以 `below_min_len` 落选、进救援候选后判 `new` 不并入）；第 25 章的分段基线就是这个会话；
- `s2-single-cross/`（201–210）：点外卖 4+3 帧，中间被回复李经理消息 3 帧打断——实体「蜀香园麻辣香锅」「¥45」跨断点延续；
- `s3-multi-cross/`（301–315）：订酒店 3+3+3 帧，被打车 3 帧、记备忘 3 帧两度打断——本章的展品；
- `s4-noise-rescue/`（401–409）：网购跑步鞋 4 帧 → 刷新闻 3 帧 → 低电量弹窗 1 帧（预期噪声）→ 支付成功尾帧 1 帧（1 < `min_len`，预期先被剔、再被救援）；
- `s5-pure-noise/`（501–505）：锁屏、广告弹窗、系统更新、误触相机、低电量——互不相关的纯噪声会话（负样本）。

fixture 由 `tools/gen_fixtures.py` 一次性确定性生成（树是唯一语义源，截图为 PIL 程序化绘制）——本章与第 25 章共用这一个工程，那边讲分段与摘取基线，这边讲缝合层。`project.toml` 的骨架（`[segment].context` 全文见 26.5——它是本工程的第一调优抓手）：
...
min_len = 2                       # 孤帧段以 below_min_len 落选 → 进 stitch 救援候选
...
trace 订阅记得加 `"stitch"` 通道（本例 `channels = ["segment", "stitch", "extract", "classify", "verify", "schema"]`）——缝合判定的审计全靠它。跑起来：
...
stderr 尾部（真实运行，退出码 0，全程约 345 秒）：

```
2026-07-17T09:18:38+08:00 INFO  emitter batch=1 批 1 落盘：主输出 +9 行（累计 9），rejects +8（累计 8）
2026-07-17T09:18:38+08:00 INFO  run     batch=1 batch.end active=9 dropped_dup=0 dropped_lowq=0 dropped_verify=0 failed=0 duration_ms=344587 fanout=0 episodes=12 absorbed=45 dropped_noise=8 stitched=3 threads=9
2026-07-17T09:18:38+08:00 INFO  emitter batch=- finalize：fsync + rename  out/stream-labels.jsonl.part → out/stream-labels.jsonl（9 行）
2026-07-17T09:18:38+08:00 INFO  emitter batch=- 已写出 out/stream-labels.rejects.jsonl（8 行）与 out/stream-labels.report.json
...
2026-07-17T09:18:38+08:00 INFO  run     batch=0 run.end exit_code=0
```

53 帧进来，主输出 **9 行**——每行一条**线索**。`batch.end` 里的两个 v1.9 新字段直接给出缝合总账：`stitched=3`（三个 episode 被并成了壳）、`threads=9`（= episodes 12 − stitched 3）。逐场景对上 26.1 的预期（会话级数字由主输出 + rejects + trace 汇出，report 是合计行）：

| 场景（会话） | 帧 | episodes | threads | stitched | seams | rescued_short | dropped_noise | absorbed | 冗余校验 threads = episodes − stitched |
|---|--:|--:|--:|--:|--:|--:|--:|--:|---|
| s1 串联 + 噪声 | 14 | 2 | 2 | 0 | 0 | 0 | 2 | 12 | 2 = 2 − 0 ✓ |
| s2 单交叉 | 10 | 3 | 2 | 1 | 1 | 0 | 0 | 10 | 2 = 3 − 1 ✓ |
| s3 多交叉 | 15 | 5 | 3 | 2 | 2 | 0 | 0 | 15 | 3 = 5 − 2 ✓ |
| s4 噪声 + 救援 | 9 | 2 | 2 | 0 | 1 | 1 | 1 | 8 | 2 = 2 − 0 ✓ |
| s5 纯噪声负样本 | 5 | 0 | 0 | 0 | 0 | 0 | 5 | 0 | 0 = 0 − 0 ✓ |
| **合计（= report）** | **53** | **12** | **9** | **3** | **4** | **1** | **8** | **45** | **9 = 12 − 3 ✓** |

五行全部与设计预期严丝合缝：串联零缝合（s1 的桌面尾帧进了救援候选池、被判 `new` 维持原判——保守面对「不该缝的」说不）、单交叉一缝一接缝、多交叉两缝两接缝、s4 救回 1 帧（`rescued_short=1`）且低电量弹窗照旧进拒绝通道、纯噪声会话零产出。s4 的接缝值得一提：救回的支付尾帧（409）与网购前半的末帧（404）之间隔着新闻线索的 3 帧（外加 1 帧噪声）——按判据构成接缝（`seams=1`），而不是「救援 = 接缝」（辨析见 26.6）。

## 26.3 机制：单调选池两遍缝合、机械先验合取与接缝标定

**按会话独立执行，会话内严格串行。**候选流 = 会话内的 episode（按会话序）+（`rescue_short = true` 时）`below_min_len` 短段按**连续 run 重组**的救援候选——注意 `reason="noise"` 的噪声帧**不入候选池**（s1 的社交消息屏、s4 的低电量弹窗、负样本会话的四帧噪声从没资格参与缝合）。池是一个串行决策过程（前一个候选的归属影响后一个候选看到的池），所以会话内逐候选处理、会话间也按批内位置顺序逐会话处理，全程零随机、事件序确定——并发只出现在 `votes > 1` 的重复采样内部。
...
**这套机制在本例里的走线**：20 次判定 = 一遍 14（12 个 episode 候选 + s1 桌面尾帧与 s4 支付尾帧的 2 个救援候选）+ 二遍 6（s1 两条、s2 一条、s3 两条、s4 一条单碎片线索的复评）；4 次并入（3 次 episode 并入 + 1 次救援命中）全部是「LLM resume + `app_overlap` ∧ `entity_overlap` 双腿命中」。s2 的那次并入（真实 trace 行，格式化展示；`…` 处省略 `ts`/`run_id`/`batch_no`）：
...
{…, "stage": "stitch", "ev": "stitch.judge", "record_ids": ["bb02cb1f4a409be0"],
 "payload": {"session_id": "66b9c1b6e5f3fe2e", "candidate": "episode", "repass": false,
   ...
   "reason": "候选首帧\"确认订单\"直接承接线索2尾帧\"去结算\"，同一App、同一商品（蜀香园麻辣香锅×1 ¥45），是购物车结算流程的自然延续。",
...
**主输出**一行 = 一条线索。s3 的展品行（真实运行产物，格式化展示；`run`/`source` 形态同第 25 章、`member_ids` 九项从略）：

```json
{
  "task_label": "预订杭州酒店并查看入住凭证",
  "app": "com.example.hotel",
  "summary": "在酒店App搜索杭州8月2日酒店，预订湖畔云居酒店大床房并完成支付，查看订单详情和入住凭证。",
  "_meta": {
    "id": "3661a64882a11c79",
    "run": {…}, "source": {…},
    "stream": {
      ...
      "member_count": 9,
      "member_ids": […],
      "member_sources": [{"file": "s3-multi-cross/uitree_301.jsonl", "pair_index": 301}, …,
                          {"file": "s3-multi-cross/uitree_315.jsonl", "pair_index": 315}],
      ...
      "steps": [                             ← 全线索连续编号 0..n−2（9 成员 ⇒ 8 步）
        {"index": 0, "action_type": "click", "target": "搜索酒店", "value": null,
         "description": "在酒店预订页面点击\"搜索酒店\"按钮，搜索杭州8月2日至8月3日的酒店", "resumed": false},
        {"index": 1, "action_type": "click", "target": "湖畔云居酒店", "value": null,
         "description": "点击酒店列表中的\"湖畔云居酒店\"条目，进入该酒店的详情页面", "resumed": false},
        {"index": 2, "action_type": "app_switch", "target": null, "value": null,
         "description": "线索接缝：被在快捷出行App呼叫快车前往城西银泰城打断后恢复", "resumed": true},
        ...
        {"index": 5, "action_type": "app_switch", "target": null, "value": null,
         "description": "线索接缝：被在备忘录App中记录购物清单等备忘事项打断后恢复", "resumed": true},
        {"index": 6, "action_type": "click", "target": "查看订单", "value": null,
         "description": "在支付成功页面点击\"查看订单\"链接，进入订单详情页面", "resumed": false},
        {"index": 7, "action_type": "click", "target": "入住凭证", "value": null,
         "description": "在订单详情页点击\"入住凭证\"入口，进入入住凭证页面", "resumed": false}]
    },
    "scores": {"coherence": 1.0, "purposefulness": 1.0, "noise_residue": 0.6,
                "completion": 1.0, "__aggregate__": 0.9,
                "mode": "pointwise", "batch_no": 1, "pool": "travel"},
    "dedup": {"kind": "unique"},
    "classification": {"label": "travel", "labels": ["travel"], "source": "llm"},
...
- 一处诚实的旁注：三条多碎片线索（s2 外卖、s3 酒店、s4 网购）的 `noise_residue` 都被打了 0.6——接缝步带着专用文案「线索接缝：被 X 打断后恢复」告知了评审（第 10 章），但 trajectory rubric 的噪声残留判据对「曾被打断」仍会压些分。没设 threshold 时这只是随行落盘的信号，后筛裁量权在你。

**拒绝通道**恰好 8 行，与 `dropped_noise=8` 对齐：s1 的社交消息屏（5，reason=`noise`）与桌面尾帧（14，reason=`below_min_len`——救援候选判 `new` 维持原判）+ s4 的低电量弹窗（408，reason=`noise`）+ 负样本会话的 5 帧（4 帧 reason=`noise`、帧 504 reason=`below_min_len`）。注意**没有**帧 409 的行——它被救援翻回 `absorbed` 了；而 `report.stream.below_min_len = 3` 数的是**发生**次数（帧 14、409、504 各一次，救援不回退计数），rejects 里却只有两行 `below_min_len`——差的那一次就是被救走的 409，两个数字讲的是同一帧的两段经历。
...
"stitch": {
  ...
  "judgments": 14,          ← 一遍判定数（12 episode 候选 + 2 救援候选）
...
左 = 9 + 0 + 0 + 0 + 8 + 0 + 0 + 45 + 3 = 65；右 = 53 + 0 + 0 + 12 = 65 ✓
```

直觉读法：右侧每个 episode 都是凭空追加的新信封（+12），左侧的壳（+3）把「一个 episode 并进另一个」的抵扣记回来——所以 `threads = episodes − stitched` 恒成立，报告里 `counts.threads` 就是按这个恒等式导出的单点。extract 的账也变了口径：`stream.extract.transitions = 32` = 36 个相邻对 −4 个接缝占位——**占位步不计入 `transitions` 与 `by_type`**（零 LLM 的机械产物不该灌污动作分布；本例 `by_type` 是 `click: 31` + `input_text: 1`，没有一个占位的 `app_switch` 混进来）。接缝的唯一计量点在 `stream.stitch.seams`。
...
```toml
[segment]
context = "手机屏幕操作录屏流；流中各项活动（购物、通讯、出行、资讯等）互为独立任务、彼此无从属或配合关系——用户常在任务间来回切换，被搁置的任务稍后可能切回收尾：切到另一任务处、切回被搁置任务处（含回到其支付/订单页完成收尾）都是新流程的开始（context_switch）；一笔任务已办完（下单成功、出票成功等收尾页）之后紧接着开始的另一笔任务同样是新流程的开始；advances 仅适用于为推进当前这一笔进行中的任务而发生的跨屏实体延续（如去短信里取验证码）——与几帧之前被打断的那笔任务的实体呼应属于切回收尾（context_switch），不是 advances；低电量弹窗、系统弹窗、通知面板、锁屏、广告弹窗等与前后操作无关的短暂插入屏属于干扰帧（interruption），不是任务切换"
```

四个要点全在里面：声明各活动**互为独立任务**（防止 LLM 把「打车去商场」脑补成「订酒店行程的一部分」而判 `advances` 不切段）；声明**切回挂起任务 = 新流程开始**（碎片由此切出，缝合由此有料可缝）；把 `advances` 钉死在**当前这一笔进行中的任务**上——办完收尾页之后的背靠背新任务、与几帧前被打断任务的实体呼应，都是 `context_switch` 不是 `advances`（这两句是对着真实漂移调出来的：早期短版 context 下，glm-5.2 偶发把 s1 的背靠背串联判成一整段、把 s4 的跨段实体呼应判成推进——语义边界句加上后同 seed 复跑稳定）；**枚举本域的弹窗噪声原型**（低电量、系统弹窗、通知面板、锁屏、广告——收敛 `interruption` 的口径）。
...
**负样本协议：拿纯噪声会话当门禁。**`s5-pure-noise` 会话在本次真跑里产生了**零次**缝合判定——四帧被判 `noise`（连候选池都进不去），帧 504 以 `below_min_len` 落选成救援候选、但**池空的救援候选零调用跳过**（救援候选永不开新线索），整个会话零判定零产出。给自己的采集管线留一段确认无任务的纯噪声流做常驻负样本：它一旦缝出线索，说明分段的噪声口径或缝合的保守面出了问题——这比任何正样本都更早暴露「过连接」倾向。
...
**成本账。**本次真跑 135 次调用（default 126 + judge 9）、约 345 秒；`per_stage_s` 里 stitch 以 111.3 秒居首——判定本身只有 20 次，但**会话内串行**是结构性的（池是串行决策过程），长会话的判定链不能并行摊薄。对照 `--dry-run` 的估算（真实输出）：
...
`stitch_calls=10` = 5 会话 ×（一遍 1 + 二遍 1）× votes 1——估算沿用 stream 的「episodes ≈ sessions」下界口径（实跑 20 次判定）；quality/annotate/verify/classify 同理报下界、extract 报上界（48 = 剔噪前相邻对数，实跑 32）。实跑比估算多出的部分 = 下界口径差 + 15 次结构修复调用（trace 里 44 条 `schema.repair` 事件，29 条在 L1 确定性层免费解决、15 条走了 L3 修复环）。`votes` 的账见 26.6 末条。

## 26.6 常见问题

**开了缝合，`--strict` 反而不红了？**可能且预期。`--strict` 的判据是「rejects 非空」；开缝合前，s4 的支付尾帧以 `below_min_len` 落在拒绝通道里；开缝合后它被救援翻回 `absorbed`，rejects 少了一行——只有这一行时结果就从 1 变 0。stitched 壳也不写 rejects（只计数）。同一份输入，「拒绝更少」是缝合把该救的救回来了——不是账目错误（第 8、15 章；本工程 rejects 里还有 8 行噪声帧，strict 仍会红——stream 工程挂 strict 的策略见第 25 章）。

**救援回来的帧和线索紧挨着，为什么有时算接缝、有时不算？**判据只有一条：**拼接对的会话序间隙里有没有归属其他线索的帧**（噪声帧不算数）。s4 的救援帧 409 与网购前半的末帧 404 之间隔着新闻线索的 3 帧（外加 1 帧噪声）——是接缝，占位步的 description 点名了打断者（「线索接缝：被在每日头条App浏览台风木兰相关新闻并查看评论打断后恢复」）。假如短段紧贴着自己线索的尾碎片（间隙里只有噪声帧、或干脆相邻），那是**真实转移**：照常送 LLM 摘取，不占位、不计 seams。「救援」说的是帧的来路，「接缝」说的是缝合处的间隙内容，两个概念正交。
