...
**口径**：**「作为语言模型训练数据的价值」**——不是「用户请求是否合理」，不是「内容是否合规」。教程二演示过：一条完全正常的请假条请求，在这套 rubric 下可能是低分（无知识含量、无专业度）。你的任务口径若是「保留真实多样的用户意图」，请自定义 rubric。
...
**口径**：**「作为 GUI 轨迹训练样本的价值」**——一段操作序列是否走完了（completion）、步步衔接（coherence）、专注一件事（purposefulness）、干净无杂帧（noise_residue）。四条准则全部**任务无关**：不问「做的是什么任务」，只锚定序列的内部结构，所以对任何 App、任何领域的 episode 都能直接用。措辞也是模态中立的——extract 关闭时没有动作序列，「步骤」读作「帧间变化」照常成立（此组合 M1 会打提示 warning）。流模式下 `quality.rubric` 缺省即解析为这一套（第 10 章）。
