...
> 三套默认评价准则随包分发，`labelkit rubric --show default:text|default:ui|default:trajectory` 可随时导出原文。
> 本附录给出全文并附「什么时候它适合你 / 不适合你」的判断依据。修改方法：导出 → 编辑 →
> 并入 project.toml（设 `quality.rubric = "inline"`，加顶层 `[rubric]` 表与必填的 `name` 键，
> 再把各 `[[criteria]]` 改写为 `[[rubric.criteria]]`）——完整通路见 B.4。
...
## B.3 default:trajectory（操作序列数据，v1.8）

**血统**：`completion` 与 `coherence` 出自 OS-Genesis 的轨迹奖励模型（TRM）——原文是 1–5 五级，此处按 LabelKit 家规改制为 0–5 六级加性量表；`purposefulness` 是从 Coherence 判据中「toward the goal」一义拆分独立而来；`noise_residue` 源自 RPA 日志分割研究的噪声处理实践（Leno et al.）——后两维无 TRM 原文直接背书。

**口径**：**「作为 GUI 轨迹训练样本的价值」**——一段操作序列是否走完了（completion）、步步衔接（coherence）、专注一件事（purposefulness）、干净无杂帧（noise_residue）。四条准则全部**任务无关**：不问「做的是什么任务」，只锚定序列的内部结构，所以对任何 App、任何领域的 episode 都能直接用。措辞也是模态中立的——extract 关闭时没有动作序列，「步骤」读作「帧间变化」照常成立（此组合 M1 会打提示 warning）。stream 模式下 `quality.rubric` 缺省即解析为这一套（第 10 章）。

| key | weight | 量的是 | 高分长什么样 |
|---|---|---|---|
| `completion` | 1.0 | 是否推进到可辨识的终态 | 末帧有确认页/成功提示等明确收尾证据 |
| `coherence` | 1.0 | 相邻步骤是否互相承接 | 步步承接、无跳变、无往复抖动 |
| `purposefulness` | 1.0 | 是否朝单一目标持续推进 | 接近达成目标的最短操作路径 |
| `noise_residue` | 1.0 | 无关步骤（弹窗/误触）的残留程度 | 全部步骤均属主线 |
...
**适合直接用**：GUI 轨迹/操作序列数据集的质量分层与筛选；分段效果本身的间接体检（noise_residue 均值低 ⇒ 上游分段没剔干净）。
**不适合直接用**：你的口径包含「任务本身的业务价值」（它刻意不量任务内容——需要的话加自定义 criterion）；非序列数据（单帧/单条文本请用前两套）。

## B.4 自定义 rubric 的完整通路
