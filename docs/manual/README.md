...
- **抄作业**：Part VI 五篇教程从易到难——教程一从空目录手搭（数据与配置全在文中），教程二至四基于仓库内 `examples/` 的可运行工程（text、ui 与 text 的纯生成变体 `project-synth.toml`；示例按输入格式分三个工程，各自开满该格式可用的算子），教程五是生产级配置模板与运维纪律（虚构场景，供改造套用）；
