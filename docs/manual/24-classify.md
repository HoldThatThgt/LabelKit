...
> 哪些参数能按类覆盖、哪些永远全局？**
> 本章样例全部来自 `examples/classify` 的真实运行。
...
## 24.2 快速上手：examples/classify 全流程

仓库自带的 `examples/classify` 是一个混合意图工程：12 条输入（5 条写作协助、1 条翻译、4 条知识问答、1 条纯笑声，外加 1 条一字不差的重复请假条），流水线开 dedup → classify → quality（pointwise）→ annotate。逐节看它的 `project.toml`。
...
**第二节：按类覆盖。**全局打分是 pointwise + 0.3 的线，然后给两个类各开小灶：
...
注意 `other` 类**什么都没配**——未出现的键一律继承全局：other 类记录用全局 0.3 的线、全局标注指令。合并优先级是 `[class.<类名>].<节>.<键>` > `[<节>].<键>` > 内置默认，M1 启动时静态合并冻结，运行期零开销。
...
```bash
cd examples/classify && mkdir -p out
...
```
   ...
   scanned=12  ingested=12  bad_input=0  generated=0
   dropped_dup=1  dropped_lowq=6  dropped_verify=0  failed=0  emitted=5
```

守恒等式照常成立：`5 + 1 + 6 + 0 + 0 = 12`。分类本身的账在 `report.json` 新增的 `classify` 节里：

```json
"classify": {
  "assignment": "single",
  "classes": {"writing": 6, "qa": 4, "other": 1},
  "fallback_count": 0,
  "failures": 0
}
```
...
{…, "ev": "quality.gate", "record_ids": ["f4450df8817e7a88"],
 "payload": {"aggregate": 0.0, "decision": "drop", "threshold": 0.3, "pool": "other"}}
```
...
multi 模式的机制要讲清楚（本节没有真实运行样例，`examples/classify` 是 single 工程；以下是规格语义）：
...
开了 classify 之后，主输出、拒绝通道与报告各多了什么（trace 的新事件与新字段在 24.2 已经见过）。**主输出**每行多两处（真实运行产物第 1 行，格式化展示）：

```json
{
  "intent": "writing_assist",
  "topic": "周报模板（本周工作、下周计划、风险与求助）",
  "difficulty": "easy",
  "_meta": {
    "id": "01733049bee5182d",
...
三个细节：① `classification` 只落 `label` / `labels` / `source` 三键——判决理由和 sc 统计不落主输出，要看去 trace（`classify.decision` 事件）；② `scores.pool` 与 `classification.label` 恒相等，pool 是打分池的自述，pairwise 模式下「批内相对分」从此变成「**池内**相对分」（见 24.7）；③ 输出 Schema 是全局的——本次真跑第 4 行是个现成例子：那条翻译请求被分进 writing 池、按写作类指令标注，但 `intent` 字段仍从全局枚举里选出了 `"translation"`。类只改工艺，不改产出结构。

**拒绝通道**每行多一个 `label` 键（真实运行产物第 2、4 行，逐字）：

```json
{"_meta": {"id": "47ccf078d1ec52f8", "source": {"file": "input.jsonl", "line_no": 3, "generated_from": []}, "stage": "quality", "reason": "below_threshold", "errors": [], "label": "writing"}}
{"_meta": {"id": "4256f928418a13a1", "source": {"file": "input.jsonl", "line_no": 7, "generated_from": []}, "stage": "dedup", "reason": "exact", "errors": [], "label": null}}
```
...
**报告**的 `quality` 节多了 `by_class`——每个类池一套独立的直方图与准则均值（顶层的直方图/均值仍保留，是全池汇总；真实运行产物按池名字典序排列，`…` 处省略 other / qa 两池的同构内容与其余全为 0 的桶）：
...
第 10 章「对着直方图画线」的流程从此**按池执行**：给 writing 类画线，看 `by_class.writing` 的直方图，别再看全池汇总——混合分布的汇总直方图会把两个类的峰糊在一起。

**下游常用姿势**（延续第 8 章的清单）：

```bash
# 按类拆分主输出
jq -c 'select(._meta.classification.label == "qa")' out/classify-labels.jsonl
# 各类产出行数（本次真跑：writing 3、qa 2）
jq -r '._meta.classification.label' out/classify-labels.jsonl | sort | uniq -c
# 按类统计淘汰去向
jq -r '._meta | "\(.label)\t\(.stage)/\(.reason)"' out/classify-labels.rejects.jsonl | sort | uniq -c
```
