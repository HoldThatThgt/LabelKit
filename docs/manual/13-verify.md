...
## 13.5 stream 模式：缺陷表评审与成员手术（v1.8）
...
`policy = "repair"` 时，缺陷驱动**成员手术**：`off_task_members` 指认的帧被**收缩**出段（改判噪声帧，进拒绝通道，`reason="off_task_member"`）；`missing_*` 走**三级回收判定**——① 缺帧还躺在批内噪声池里 ⇒ 复裁确属本流程即**回收**进段；② 缺帧已被相邻 episode 占用 ⇒ **只标记不动手**（不做跨段仲裁）；③ 无处可寻 ⇒ 缺陷条目补标 `capture_gap`（采集本身缺帧，修不了）。`wrong_stitch`（v1.9）走**独立分支：只标记不拆线**——它不进入 `missing_*` 的回收扫描，也没有自动拆线手术，评审认定缝错即按 fail 处置；错缝的正确修法是回到 `[stitch]` 调参（收紧偏置、开 votes）重跑，见第 26 章。手术之后：接缝处的动作重新摘取、动作序列重新编号、重标注、再复审——轮数照常计入 `max_repair_rounds`。
