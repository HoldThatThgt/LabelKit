...
开启分段算子（第 25 章）后，评审对象是整个 episode（动作序列 + 标注），verify 的输出多一张**缺陷表**——不止说 pass/fail，还要指认「哪里坏了」，五类缺陷：
...
为让评审能发现「缺头缺尾」，评审证据专门带一段 **[边界余量]**：段边界外前后各 2 帧的摘要及其去向（被判噪声 / 属相邻段 / 不存在）——评审看得见「门外」有没有本该收进来的帧。

`policy = "repair"` 时，缺陷驱动**成员手术**：`off_task_members` 指认的帧被**收缩**出段（改判噪声帧，进拒绝通道，`reason="off_task_member"`）；`missing_*` 走**三级回收判定**——① 缺帧还躺在批内噪声池里 ⇒ 复裁确属本流程即**回收**进段；② 缺帧已被相邻 episode 占用 ⇒ **只标记不动手**（不做跨段仲裁）；③ 无处可寻 ⇒ 缺陷条目补标 `capture_gap`（采集本身缺帧，修不了）。手术之后：接缝处的动作重新摘取、动作序列重新编号、重标注、再复审——轮数照常计入 `max_repair_rounds`。
