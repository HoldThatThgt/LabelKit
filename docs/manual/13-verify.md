...
- v1.11 预算注：评审团共用**同一份**评审 prompt（一次构建、广播全团），所以声明了 `context_window` 时记录侧份额按团内**最小**的 `input_budget` 装填——最小窗口的评审装得下，全团都装得下（对照：quality 的 pairwise 是逐评审各自构建、按各自预算装填，第 10 章）。
...
为让评审能发现「缺头缺尾」，评审证据专门带一段 **[边界余量]**：段边界外前后各 2 帧的摘要及其去向（被判噪声 / 属相邻段 / 不存在）——评审看得见「门外」有没有本该收进来的帧。v1.9 开启缝合后，证据再新增一段 **[片段结构]**（仅 stitch 启用时渲染）：逐碎片给出线索内序数、帧跨度与首帧摘要，外加接缝位置表——没有这一段，评审无从判断「这条线索是不是缝错了」，`wrong_stitch` 的可判性全靠它。
