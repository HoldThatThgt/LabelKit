...
| `supports_vision` | false | 声明该模型能看图。**UI 模态下被引用的 profile 必须为 true**——这是启动时的硬校验（填 false 会退出码 2），因为跑到一半才发现模型看不了图，钱已经烧了。v1.8/v1.9 stream 模式（第 25、26 章）下这条校验有四处例外：`extract.llm` 引用的档**恒**要求 true（每次摘取都看前后两帧截图）；`segment.llm` 仅 `segment.use_vision = true` 时要求；`quality.llm` 反而**免除**要求（序列打分是纯文本，UI 模态也不看图）；`stitch.llm`（v1.9）同样**恒不**要求（缝合判定的证据是摘要卡，纯文本无图） |
