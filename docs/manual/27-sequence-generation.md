...
verify、最终行装配、replay 与 candidate-local CrossView 可跨槽并发；每个 declared slot 先完成并验收 baseline，再让
彼此独立的 counterfactual suffix 结构化并发，按 variant 声明序归并。同一 branch 内事件仍按 `state_after` 串行；
任一 suffix 主动取消时，会先清理同组 siblings，再把原始取消传出。
