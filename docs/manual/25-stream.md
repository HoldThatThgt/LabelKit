...
仓库自带的 `examples/stream`（`project.toml`）是一个 53 帧、五个场景子目录的 UI 操作流工程——时序流格式能开的算子全开（分段、缝合、去重、分类、摘取、轨迹打分、序列标注、评审修复；generate 与 stream 互斥）。本章用它的第一个会话 `s1-serial-noise/`（帧 1–14）讲 v1.8 的分段与摘取基线：任务 A「点外卖」帧 1–8（其中帧 5 是突然插入的社交 App 消息屏——预期噪声）、任务 B「打车」帧 9–13 背靠背、帧 14 回到桌面；其余四个会话是穿插与救援场景，属第 26 章缝合的舞台。fixture 由 `tools/gen_fixtures.py` 一次性确定性生成（树是唯一语义源，截图为 PIL 程序化绘制），刻意埋了「实体跨屏延续」的线索：餐厅名「川味麻辣烫」跨帧 3/4/6 出现、金额 ¥32 跨帧 4/6/7。逐节看 `project.toml`。
...
`[stream]` 声明「帧流怎么排、会话在哪断」：本例用分区键 `source_dir` 让**每个子目录成为一个会话**。`[segment]` 是 stream 模式总开关；`window = 16` ≥ 最长会话，滑窗退化为**每会话恰一窗**（窗小于会话时按步长 = window−1 滑动、重叠 1 帧）；`context` 只是可选域上下文——**边界判据内置于固定模板，零配置可用**，这行不是必需品。
...
stderr 尾部的终版摘要（真实运行，退出码 0，全程约 345 秒）：
...
53 帧进来，主输出却只有 **9 行**——这不是丢了数据，是换了记账单位：45 帧被吸收进 12 个 episode（状态 `absorbed`，其中 3 个 episode 又被缝合并壳，第 26 章）、8 帧成了噪声（`dropped_noise`），守恒等式的完整验算见 25.4。聚焦 s1 会话：14 帧产出 2 行，与人工预期严丝合缝——任务 A 成一段（7 成员，噪声帧 5 被剔）、任务 B 成一段（5 成员）、帧 14 落进 rejects。
...
**主输出**一行 = 一个 episode（缝合开启时 = 一条线索——单碎片线索就是原样的 episode）。真实运行产物第 1 行（s1 的任务 A，格式化展示；`steps` 的六步全文照录）：

```json
{
  "task_label": "在美食外卖App搜索并下单川味麻辣烫招牌麻辣烫1份，金额¥32",
  "app": "com.example.food（美食外卖）",
  "summary": "搜索麻辣烫，在川味麻辣烫店铺下单招牌麻辣烫1份，支付¥32元完成外卖订单。",
  "_meta": {
    "id": "71ade2027644e5e2",
    "run": {…},                              ← 与既有形态一致，从略
    ...
    "stream": {
      ...
      "thread_id": "71ade2027644e5e2",       ← v1.9 键：仅 stitch 开启时在场（第 26 章）
      ...
      "fragments": [{"order_span": [1, 8], "member_count": 7, "cause": "origin",
                      "source_episode": "71ade2027644e5e2"}],
                                             ← v1.9 键：碎片装订记录，单碎片 = 没缝过（第 26 章）
      "steps": [                             ← extract 产物；关 extract 时恒 null
        {"index": 0, "action_type": "click", "target": "搜索美食", "value": null,
         "description": "点击搜索框进入搜索页面", "resumed": false},
        {"index": 1, "action_type": "click", "target": "麻辣烫", "value": null,
         "description": "在热门搜索中点击\"麻辣烫\"标签，进入麻辣烫搜索结果页面", "resumed": false},
        {"index": 2, "action_type": "click", "target": "川味麻辣烫", "value": null,
         "description": "在搜索结果列表中点击\"川味麻辣烫\"餐厅，进入该餐厅的详情页面", "resumed": false},
        {"index": 3, "action_type": "click", "target": "加入购物车", "value": null,
         "description": "点击\"加入购物车\"按钮，将招牌麻辣烫加入购物车，页面随后跳转到购物车界面", "resumed": false},
        {"index": 4, "action_type": "click", "target": "去结算", "value": null,
         "description": "用户点击购物车页面的\"去结算\"按钮，进入确认订单页面", "resumed": false},
        {"index": 5, "action_type": "click", "target": "提交订单 ¥32", "value": null,
         "description": "点击\"提交订单\"按钮，提交订单后跳转到下单成功页面", "resumed": false}]
    },
    "scores": {"completion": 1.0, "coherence": 1.0, "noise_residue": 1.0,
...
逐键读 `_meta.stream`：`member_sources` 是完整成员溯源（每帧来自哪个文件哪个 index——`source` 键只继承首成员），拿它能把 episode 还原回原始帧；`order_span` 与 `member_count` 对不上（跨度 8、成员 7）就说明段内有帧被剔了。`thread_id`、`fragments` 与步行内的 `resumed` 是 v1.9 增键，**仅本工程开着 `[stitch]` 才在场**（读法在第 26 章；关掉缝合，这三处消失，主输出与 v1.8 逐字节等价）。顶层三个字段仍是你的 Schema 产物——**输出结构照旧由全局 Schema 管**，stream 改变的只是「一行代表什么」。另两处细节：`verification` 在 stream 模式恒带 `defects` 键（无缺陷 = 空数组）；这行的 `purposefulness` 被打了 0 分（聚合分 0.75）——对一条目的明确的下单流来说这是个可疑判决，但因为没设 threshold，它只是个随行落盘的分数。**stream 工程默认只打分不筛**的价值就在这：判分的噪声不会变成数据的损失，后筛时你还有机会用 trace 复核。

**拒绝通道**是噪声帧的去向（`rejects = "full"` 档；s1 的两行 `_meta` 逐字如下，`record` 载荷——该帧的树文本与图路径——以 `{…}` 略去）：

```json
{"_meta": {"id": "c51c341656eb8447", "source": {"file": "s1-serial-noise/uitree_5.jsonl", "pair_index": 5, "generated_from": []}, "stage": "segment", "reason": "noise", "errors": []}, "record": {…}}
{"_meta": {"id": "47d1c7373d1fa7fb", "source": {"file": "s1-serial-noise/uitree_14.jsonl", "pair_index": 14, "generated_from": []}, "stage": "segment", "reason": "below_min_len", "errors": []}, "record": {…}}
```

两个 reason 别混：帧 5 是 LLM 判的 `interruption`（reason=`noise`，社交 App 消息屏）；帧 14 是「`context_switch` 开了新段、但段里只有它自己（1 < min_len=2）」的 `below_min_len`——桌面屏不是噪声，只是不够成段。审计噪声率时只数 `noise`，别把 `below_min_len` 算进去。verify 手术收缩逐出的帧则是第三种组合：`stage="verify", reason="off_task_member"`。
...
"counts": {
  ...
  "failed": 0, "generated": 0, "emitted": 9,
  "episodes": 12, "absorbed": 45, "dropped_noise": 8,
  "stitched": 3, "threads": 9
}
```
...
代入验算：左 = 9 + 0 + 0 + 0 + **8** + 0 + 0 + **45** + **3** = 65；右 = 53 + 0 + 0 + **12** = 65。✓ 直觉读法：右侧 `+ episodes` 是因为每个 episode 都是凭空追加的新信封（与 classify 扇出的 `fanout` 同构），左侧 `absorbed + dropped_noise` 则是原始帧的两种新去向（`stitched` 的壳记账在第 26 章展开）。新增的 `stream` 节（真实产物，`by_type` 其余 9 个动作类型本次全为 0、以 `…` 略；`stitch` 子块留给第 26 章）：

```json
"stream": {
  "sessions": 5, "episodes": 12, "mean_episode_len": 3.75,
  "absorbed": 45, "dropped_noise": 8, "below_min_len": 3,
  "digest_poor_frames": 0, "segment_failures": 0,
  "stitch": {…},
  "extract": {"transitions": 32, "fallback_steps": 0, "failures": 0,
               "by_type": {"click": 31, "input_text": 1, …}},
...
对账三连：`transitions=32` = 各线索 Σ(成员数 − 1) = 36 再减去 4 个接缝占位步（占位不计入摘取账，第 26 章）；`dropped_noise=8` 里有 2 条是 `below_min_len`（独立计数拆给你看——`below_min_len=3` 是**发生**计数，第三次命中的帧被缝合救援翻回了 `absorbed`，第 26 章）；`mean_episode_len=3.75` = 45 成员 ÷ 12 段。`fallback_steps` / `segment_failures` / `verify.defects` 全为 0——这是一次干净的运行，这些计数器不为零时的读法在 25.5。

## 25.5 调优与审计闭环

**三个旋钮，按影响面排序。**① `gap_s` / `gap_steps`（会话粒度）：gap 偏大 = 欠分割，还有 LLM 精化兜着；gap 偏小 = 过分割，**段一旦切碎就再也拼不回来**（LLM 只在会话内精化，v1.9 的缝合算子同样只在会话内缝——跨会话永远无解，第 26 章）——这就是 `gap_s` 默认给到 300 秒偏大值的结构性理由，宁欠勿过。② `segment.window`（滑窗宽度）：窗内上下文越足判得越稳，业界证据甚至偏向「整段单调用」形态——会话普遍不长时直接把 window 调到 ≥ 会话长度，滑窗天然退化为整段单调用（本工程 `window=16` 就是这么定的）；窗小步多则调用省不了几个、接缝还多。③ `segment.context`（域上下文）：告诉审核员「这是什么流」（本工程的长版 context 枚举了低电量弹窗、通知面板等噪声原型，还声明了任务互斥与切回语义——逐要点解读在第 26 章），它不定义边界，但能收敛噪声与切换判定的口径。
...
```json
{"ts": "2026-07-17T09:13:06.470+08:00", …, "stage": "segment", "ev": "segment.boundary",
 "payload": {"session_id": "f00e41052479a460", "window": [0, 14],
   "relations": [{"index": 0, "relation": "continues"}, …,
                 {"index": 4, "relation": "interruption"},
                 {"index": 5, "relation": "continues"}, …,
                 {"index": 8, "relation": "context_switch"}, …,
                 {"index": 13, "relation": "context_switch"}],
   "model": "glm-5.2",
   "reason": […, "切换到社交App查看新消息，与外卖点餐流程无关的短暂插入",
              "回到美食外卖购物车页面，川味麻辣烫实体延续，继续之前的点餐流程", …,
              "外卖任务已完成，切换到打车App开启全新的出行叫车流程", …]}}
```

index 4（帧 5）的 `interruption`、index 5（帧 6）的非边界回归与 index 8（帧 9）的 `context_switch` 正是 25.3 那张词表的活例：审核员在帧 6 的理由里写明了实体延续（本次判 `continues`；同为非边界值，跨 App 回归时才需要 `advances` 撑住不切段），噪声帧没有把任务切成两半，而外卖收尾后的打车 App 才真正开了新段。对边界不满意的调参循环：改 `context` / 调 `window` / 动 gap → 同 seed 重跑 → diff 两次的 boundary 事件。
...
| 来源 | 次数 | 本次真跑 |
|---|---|---|
| segment | Σ ceil((L−1)/(w−1))，L≥2 的会话；rules/孤帧计 0 | 五会话各 1 窗 = 5 |
| extract | Σ(L−1) 报**上界**（剔噪后实际 = Σ(成员数−1)，接缝占位不计） | 估 48、实际 32 |
| quality / annotate / verify | 记录基数变为 episodes/线索（估算以会话数报**下界**） | 9 线索 ×4 准则 = 36；9；9 |

`--dry-run` 的估算行无条件打印 `segment_calls` / `extract_calls` / `stitch_calls`（v1.9 增，未启用恒 0），本工程估算 `total=98`。实跑合计 135 次调用（default 126 + judge 9）、约 345 秒——高出估算的部分 = 下游按会话数报下界的口径差 + 缝合判定的实际次数（第 26 章）+ 15 次结构修复环调用（trace 里 44 条 `schema.repair` 事件：29 条在 L1 确定性层零调用解决、15 条走了 L3 修复环），估算历来不含修复。`per_stage_s` 里 stitch（111.3 秒，账在第 26 章）与 quality（99.9 秒）是两个大头——第 17 章「quality 是大头」的结论在 stream 下依然成立，extract（44.0 秒）第三，episode 越长它占比越高。
...
**孤帧会话去哪了？**不会静默消失。`len(session) == 1` 的会话走 rules 退化：原样成一个单帧 episode（零 LLM 调用），**不经 min_len**——min_len 只砍「LLM 精化切出的短段」。所以帧 14 那条 `below_min_len` 的完整因果是：它在 14 帧大会话里被判 `context_switch`、开了一个只有自己的新段，段长 1 < 2 才被丢（本工程开着缝合，它随后还进了救援候选池、被判 `new` 维持原判——救援候选永不开新线索，第 26 章）——假如它自成一个会话（比如配了 `gap_steps` 且序号断开），反而会原样活成 episode。
