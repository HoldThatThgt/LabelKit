...
> 状态：**superseded（v1.17 Wave 8 已收口）**。本文保留 v1.14 的历史决策与实现依据；当前
...
> **v1.15 交叉引用（2026-08-19）**：本版档位表**全局唯一**（所有参与序列类共享同一套 `tier_rank` / `weight` / `frame_classes`）——「每个序列类独立配置档位」已由 **spec v1.15「按类档位表」核销**（`[[class.<name>.generate.tiers]]`：表级原子覆盖 + 全局表为锚 + `tier_rank` 收窄为类内身份 + 报表条件化类嵌套），开发规格见 `docs/dev/SPEC-per-class-tiers.md`。本文其余非目标照旧停放，历史规格正文不改。
