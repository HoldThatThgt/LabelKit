...
完成并通过 expected-violation 验收后，不同 counterfactual suffix 由 branch coordinator 结构化并发，真实调用仍逐次
取得准确的 ResourceManager 许可，不把跨多次调用、可能跨 profile 的完整 branch 冒充单个 profile 叶任务。baseline
验收失败时 suffix 调用数必须为零。结果与可恢复失败按 variant 声明序归并；suffix 直接抛 CancelledError 或 task
自取消时，先取消并回收 siblings，再恢复原始 CancelledError；fatal/control 同样等待 sibling cleanup 后原样传播。
quality → annotate → verify 的阶段屏障
