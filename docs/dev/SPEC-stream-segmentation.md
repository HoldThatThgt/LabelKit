...
新：`test_stream_ingest.py`（时间戳解析/单调性/会话闭合/装箱/limit）、`test_segment.py`（摘要/diff/Schema 形状/缝合确定性/②b/min_len/守恒）、`test_extract.py`（Schema/fallback 留痕不写 errors/转移数不变量/by_type）、`integration/test_stream_llm.py`（真实 glm-5.2：边界词表内/噪声判定/动作摘取/25→20 帧降采样成功）。
...
| 2 | M8 三 Schema + M2 会话化 + M10 装箱/计量/守恒 | test_schema_engine/test_stream_ingest/test_ingest/test_orchestrator 全绿 |
