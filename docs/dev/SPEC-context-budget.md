...
| 键 | 类型 / 默认 | 说明与约束 |
|---|---|---|
| `context_window` | int / `0` | 模型上下文窗口（token）。`0` = 未声明：该 profile 上下文预算关闭（行为与 v1.10 一致），被启用阶段引用时 M1 WARN 一次。> 0 时须满足 `context_window > max_output_tokens + margin`，否则 CONFIG_ERROR（预算非正）；`margin = max(256, ceil(0.10 × context_window))`。**声明部署实效窗口，勿照抄文档（V26/[C-59]；P6 实测修正 E2E #16——文档的 `[1m]` 后缀说法与端点实况相悖，裸 `glm-5.2` 实测即 2^20 合并判定）**：同名模型随部署差数倍——Together 256K、vLLM 由 `--max-model-len` 决定；不确定时**欠声明恒安全**（只多裁不溢出）。 |
...
1. **segment 窗口（M14）**：会话级预计算逐帧 digest → 贪心切窗（重叠 1 帧、
   ...
   **CONFIG_ERROR**（M1 报错优于运行期逐会话/逐修复爆错；护栏保证**先验计价**
   下的运行期装填与复裁不失败——校准值超先验（V19 无钳制，合法）或退化个案时，
   装填器对预算收不进 2 帧的窗**强制 2 帧封窗**（V10 语义最小单元），真实 est
   超预算由 M9 终检以记录级 `context_overflow` 经既有单窗失败路径处置，永不
   run 级、永不死循环，v1.11 审计修订）；`w_min == floor` → WARN（窗数放大
   警示）。`est_static_system` 的 segment 冻结常数覆盖**全部**最坏静态骨架
   （V22 segment 例外），对任意配置 ≥ 运行期 `_static_prompt_est`——护栏静态项
   不小于装填器实际扣除项，先验保证由此成立。estimate
...
9. **M8 L3 修复调用（V25① 钉死）**：机制 = 捕获修复 complete() 抛出的
    ...
    overflow_records；不截断修复原文——截断即破坏修复语义）；吞点即该异常
    终局——reactive-400 在此经共享 `budget.feed_reactive_terminal` 补喂
    熔断恰一次（A7 盲点修订，v1.11 审计）。
...
| kind | 语义 | 处置 |
|---|---|---|
| `context_overflow` | 三形态（V24）：precheck = 终检命中（V16）/ 最小单元不装（V10）；reactive-400 = 预算开启下嗅探命中且 V20 降级耗尽；reactive-200 = `model_context_window_exceeded` 且降级耗尽（或无降级面） | 记录级 `failed` → rejects；计 `report.budget.overflow_records`。熔断矩阵：**precheck 不计连击**（无 provider 交互）；**reactive-400 终局计入连击**（A7 已裁——由**终局吞点**经共享 `budget.feed_reactive_terminal`（`_breaker_fed` duck 标防重喂）补喂恰一次：属主算子 reject 位、M8 L3 修复短路吞点、M7 回收复裁 mark-only 吞点三类（后两类为 A7 盲点修订，v1.11 审计——SchemaViolation 终局与复裁降级均不经算子 overflow reject 位），M9 抛出时不喂；成功降级/任何成功调用清零连击）；**reactive-200 终局不补喂**（HTTP 交互成功、streak 已被该次 ok 清零——`llm.call` 事件维持 status="ok"，实现者不得"修正"为 fatal，F9）。均不烧常规重试（V20 降级重试独立计数、有界） |
...
| `docs/manual/07-project-toml.md` | :56 `ui_tree_max_chars` 行——绝对上限 + 预算下动态收缩（§3.3③）；**:40 `batch_size` 行补「从不影响单次 prompt 体积」误解防护句（V14 手册半，完备性审计补录）** |
...
- [x] docs/CONTRACTS.md 全部落点（§3.8 行：镜像 ×3、rule 33/34、§10.9 注、budget 新节、complete() 条款、错误类、build_segment_prompt/build_annotate_prompt/annotate_record 签名、§7.8 V23 三面、§9.2/§9.3 登记、frame_digest 两处 V4 文案）
- [x] docs/dev/SPEC-stream-segmentation.md S12/S22/S30/S32 交叉注
- [x] `uv run python tools/build_design_doc.py --pdf` 重建渲染物
- 出口门：spec 与本 SPEC §2/§3 逐决策比对无缺漏（V1–V27 每项在 spec 正文有落点）✅（2026-07-23 完备性审计两轮，残项两枚已闭合并回录 §3.8）

**P1 公共层基础**（M1 + budget.py + errors）——完成 2026-07-22
- [x] model.py/loader.py：context_window ×2、default_image_px、vision_resolved、V2 定向报错（原始节探针）、V3 集合改造、V5/V6/V13③ 告警预检、V9 护栏（floor 2/3，F14 条件）、收尾 replace 冻结
...
- [x] complete() 终检（V16 precheck）；终止原因归一解析（V11：length/max_tokens + 双协议 model_context_window_exceeded——[C-57][C-58]）；LLMResponse.finish + _result_usage 适配（F9）
- [x] V20 超窗错误体 pattern 集（[C-75] 种子 + 可扩；完整 resp.text 匹配；预算门控）；overflow/截断不喂 fatal（F5：M9 抛 reactive 不喂、属主算子终局补喂——A7）
- [x] 每响应喂校准样本（usage 缺失兜底）；PromptBundle.image_px 载体 + 图片编码生效 px（V18/V21/V23①）；LLMClient.calibrator 自持（V23②）
- 出口门：`uv run pytest tests/common/runtime/test_llm_client.py -q` 全绿（终检先于网络、解析纯函数级、pattern 集钉死）✅

**P3 算子装填改造**——完成 2026-07-23
- [x] segment.py：digest 前移 + `_pack_windows` 贪心装填（重叠/接缝不变）+ V20 窗对半改切重试（终局补喂）+ vision_resolved 判据 + 贫瘠文案 + windows 计数
- [x] quality/classify/verify/pointwise：树渲染动态上限（§3.3③④，per-judge / min-over-panel——V25②）；序列步骤行 edges 裁剪（⑤）；不可裁块计入 est（V25③）
- [x] annotate.py：份额定序 + k_eff（⑥）+ V20 减帧重试 + V21 升级换档（F3 追加尾参）
- [x] generate.py 种子尾丢（⑦）；dedup.py embed 截断（V15）；schema_engine L3 溢出短路（⑨/V25①）
- [x] verify.py：V21 触发（fail ∧ repair → 阶梯参数传 annotate 修复面）
- [x] 全算子错误分类器 overflow/truncated 分支（V27①）+ truncations 计数
...
**P6 集成与验收（真端点 glm-5.2，需 .env）**——完成 2026-07-23（E2E-FINDINGS #15–#21）
- [x] z.ai 超窗错误体采集 → V20 pattern 断言 + E2E-FINDINGS 记录；**实测未加后缀 glm-5.2 实效窗与 `[1m]` 行为（V26）**
- [x] 小声明窗口 → context_overflow rejects、run 存活、A7 连击语义断言；校准收敛断言（report.budget.image_cost ≈ usage 反推）
