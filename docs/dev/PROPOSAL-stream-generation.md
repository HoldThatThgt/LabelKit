...
> 2026-08-13。需求（用户原文）：「希望 LabelKit 生成文本型的动作序列，并做序列级标注：需要生成文本原子，文本原子具有完备的 JSON Schema，也可以简单提供帧级生成 prompt，但是每帧可能是不同的类别和场景内容，Schema 按照类别不相同；需要文本原子数据组成多种序列，针对序列又要求独立的标注 JSON Schema，每种序列的 Schema 各自独立；需要能够在序列中插入噪音帧，需要能够生成交叉的不同序列（原子时间流数据）。」
> **状态：已 SPEC 化（2026-08-13）。**最终开发规格见 `docs/dev/SPEC-stream-generation.md`——三方预实现审计（可行性/文档清单/对抗反证）折入其 §2 裁决表，**凡与本文不一致处以 SPEC 为准**（含：直装评审判决形、S29 扩展、序列类约束按形态放宽、golden 冻结锚不动、cross_ratio 改 sessions 定容等修正）。本文保留为方案论证与决策溯源材料。问题验证：五路并行侦察（M6 generate 现状 / 流模式数据面 / Schema 面 / 约束编排观测面 / 业界调研）于 2026-08-13 完成，结论一致——该能力当前不存在，且早被 spec §8.3 O3 明文预留为「另行立项、届时须先裁决与 stream 模式的组合语义」（`spec/80-ch8-nongoals-roadmap.md:28`）。本文即该裁决的对齐材料。
