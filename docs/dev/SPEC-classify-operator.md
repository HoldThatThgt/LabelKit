...
> **状态：已实现（2026-07-08，v1.7 合入 main）。**§4 清单已全部落地（spec/ 18 文件、CONTRACTS、手册 27 章、代码与测试、examples/classify）；§7 九个开放问题全部按默认裁决实现。本文档保留为 v1.7 的开发规格与裁决记录。
...
| M4 分池打分 | feasible_with_frictions | 7 | pairing→Bradley-Terry→归一化→门控已全部按 items 子集参数化，分池只需 run() 套一层池循环；摩擦集中在计数器/报表/trace 的池维度 |
...
| # | 名称 | 问题 | 裁决 |
|---|---|---|---|
| R1 | 裁决·Schema 不写 uniqueItems | multi Schema 的 `uniqueItems` 会被 OpenAI strict 模式与部分约束解码网关硬拒（结构化输出层无条件透传） | **Schema 不写 uniqueItems**；重复标签由 classify 代码在 M8 验证后确定性归一化（去重，属已验证集合的收窄），内部 Schema 关键字集保持零增量 |
| R2 | 裁决·标注签名增 label 形参 | 类标签穿不过 M5 冻结签名（`build_annotate_prompt` / `annotate_record` 只见 frozen Record；multi 克隆共享 record.id，映射法不可行） | 两函数各增末位可选形参 `label: str \| None = None`（默认 None = 现行为，旧调用点零改动）；CONTRACTS §7.4 与 §12.11 修订。M5/M7 stage 层从 `item.classification.label` 传入 |
| R3 | 裁决·评审任务指令按类取值 | verify 提示词的 `[任务指令]` 段嵌入 `cfg.annotate.instruction`（`verify.py:99/104`），按类标注 + 全局评审指令是语义 bug | `build_verify_prompt` 增 `label` 形参（该函数不在契约内，零契约成本）：`[任务指令]` 与 `extra_criteria` 均取类有效值；`_judge_round`/`_reannotate` 透传 label |
| R4 | 裁决·fallback 留痕不写 errors | `on_error="fallback"` 若写 `item.errors` 会污染 rejects 归因（`_reject_stage_reason` 取 `errors[0]`，冻结规则） | **fallback 留痕不写 `item.errors`**：改放 `Classification.detail`（含 kind 与消息）+ error trace 事件 + 计数器 `classify.fallback`；`item.errors` 仅 `on_error="fail"` 时写。emitter 归因规则不动 |
| R5 | 裁决·rejects 与事件增 label 键 | multi 扇出后同 record.id 的兄弟信封在 rejects 与逐记录 trace 事件中不可区分 | classify 启用时：rejects `_meta` 增 `label` 键（修订 §9.2 封闭五键枚举→六键）；逐记录事件（`annotate.done`/`verify.verdict`/`quality.gate`/`error`）payload 增 `label` 字段（§8.1 契约本就只增） |
| R6 | 裁决·选择组合并 | 互斥键继承碰撞：全局 threshold + 类设 top_ratio（或反向）逐键 replace 后两键并存 → 按提案原文校验即误报 | **按「选择组」合并**：类显式提供 selection/threshold/top_ratio 任一 ⇒ 合并视图剔除全局侧互斥对键；合并期维护逐键 provenance；互斥校验跑在合并后视图上 |
| R7 | 裁决·类 rubric 合并重解析 | per-class rubric 不是逐键 replace：selector 字符串与解析产物分居两字段，inline 解析未函数化 | 抽 `_resolve_rubric(col, selector, raw, label)` helper；per-class 合并 =「合并 selector → 重解析」；pointwise 6 级校验跑在「类有效 mode × 类有效 rubric」组合上；`[class.X.rubric]` 在场但该类 selector 非 inline ⇒ 忽略并 warning（同全局惯例） |
| R8 | 裁决·关开关留配置仅警告 | `classify.enabled=false` + `[class.*]`/`[[classify.classes]]` 在场：提案定 CONFIG_ERROR，与仓库 no-op 键分级惯例（warning）冲突 | **改为 warning**（一次、点名被忽略的表），对齐 top_ratio 未生效等家族先例；「留配置、关开关」合法 |
| R9 | 裁决·fanout 计量归编排器 | `counts.fanout` 所有权：扇出发生在 classify 内，但 `counts.*` 仅 M10 可增（冻结） | M10 在 `_process_batch` 链循环对 classify 阶段记 `len(batch)` 前后增量并计数（与从 generate 返回值计 generated 同构）；契约 §11 所有权表加一行 |
| R10 | 裁决·残差公式加 fanout 项 | 熔断部分交付的 `unprocessed` 残差公式（`orchestrator.py:420-423`）未加 fanout 项，multi 下会少报 | counts 块按 `assignment=="multi"` 条件挂 `fanout` 键；残差公式右侧同步 `+ fanout` |
| R11 | 裁决·分类调用量估算修正 | 提案的 `classify_calls = total_records × …` 与继承语义矛盾（回流子批跳过 classify） | 修正：process 模式 = `ingested × max(1, sc)`；generate_only = `生成记录数 × max(1, sc)` |
| R12 | 裁决·平局计数按池升维 | 同名 criterion 跨类 rubric 会使 tie 计数器与 report 统计混池 | classify 启用时计数器键升维为 `quality.tie_outcomes.<pool>.<crit>`（tie_comparisons 同）；M10 直方图/均值累加器升维为 (pool, crit)；report 增 `quality.by_class`；关闭时键格式与报表形状不变 |
| R13 | 裁决·分池两阶段预抽派发 | 「类池按字典序处理」的并发-确定性歧义：逐池串行 await 损吞吐，池协程并发则 rng 消费序不定 | **两阶段**：同步循环按类名字典序为每池预抽配对计划（消费 ctx.rng），再把各池 LLM 调用合并为一个 gather（跨池满并发）。`_run_pairwise` 拆 plan/dispatch 两相 |
| R14 | 裁决·报表按类视图并列 | report 顶层 `quality.mode/rounds` 在 per-class 覆盖下失真；tie_rate 输出以全局 mode 为门 | 顶层字段保留（= 全局继承基值）；`by_class` 每池携带有效 mode/rounds；tie_rate 门改「存在 pairwise 池」 |
| R15 | 裁决·池级失败隔离 | quality 批级 internal-error 兜底（`quality.py:278-289`）会连坐其他池 | try/except 移入池循环：池级隔离，A 池失效不波及已完成的 B 池 |
| R16 | 裁决·打分事件增 pool 字段 | `quality.bt_fit` 事件 record_ids 恒空，分池后不可归因 | classify 启用时 `quality.bt_fit`/`quality.gate`/`quality.judgment` payload 增 `pool` 字段（只增） |
| R17 | 裁决·生成计划携带类名 | M6 规划器单池单指令结构；类归属穿不过 `postprocess_samples` 的扁平返回值 | `CallPlan` 增 `class_name` 字段；`one_call` 按 plan.class_name 取类有效 instruction/temperature；`postprocess_samples` 返回 `list[tuple[Record, str \| None]]`；`run()` 构造 PipelineItem 时带 `source="inherited"` 的 Classification |
| R18 | 裁决·类段字典序拼接 | 类间调用序未定义（预抽可复现性） | **字典序段拼接**：参与类（有种子的类）按类名字典序占据连续全局调用序号区间；预算 `C_c = ⌈len(seeds_c) × num_per_record_c / num_per_call⌉`；单遍 i=0..C−1 预抽——llm 照旧按全局序号（round_robin 零 rng 消耗 / weighted 逐 i 一次 choices），style 从**该 i 所属类**的有效 styles 均匀抽；种子抽样按全局序号升序逐调用。classify 关闭 ⇒ 单一匿名段 = 现行为 |
| R19 | 裁决·种子门槛按类默认链 | 种子门槛的按类默认链未定义 | 每类：全局 `seed_min_score` → 缺省取**该类有效** `quality.threshold` → 再缺省取**该类种子池**聚合分中位数；`select_seeds` 按 label 分组返回 |
| R20 | 裁决·批事件增 fanout 字段 | `batch.start.size` 为扇出前基数，与 batch.end 各态计数和不再相等 | 语义定为「批入口信封数」；`batch.end` payload 增 `fanout` 字段（只增） |
| R21 | 裁决·补分类采样温度键 | 提案漏 `classify.sc_temperature` 键（写了「同 annotate.sc_temperature 机制」但键表没有） | 补键 `classify.sc_temperature`（float，默认 0.7，仅 self_consistency ≥ 3 生效） |
| R22 | 裁决·契约扇出例外措辞 | Stage 契约例外措辞歧义（「不修改既有元素」与写 status/classification 矛盾） | 契约多标签扇出例外采用精确措辞，见 §3.2 |
| R23 | 裁决·配置全必填风格 | `ResolvedConfig` 全必填风格 vs 新字段默认值 | 保持**全必填无默认**风格（新增 `classify`、`class_views` 两必填字段）；直接构造 ResolvedConfig 的 ~14 个测试文件做机械补参 |
| R24 | 裁决·三处引用集补分类 | 三处 profile 引用集独立枚举（密钥解析 / 视觉必需 / probe），漏一处即运行期失败 | `loader.py` referenced 集、vision_users、`cli.referenced_profiles()` 三处各加 classify 分支，测试三处各覆盖 |
| R25 | 裁决·类节白名单外键报错 | 顶层未知键现仅 warning；`[class.*]` 白名单要求 error | spec 3.1.4「未知键报 warning」行加例外句：`[classify]`/`[class.*]` 显式接管，`[class.*]` 内白名单外键报 CONFIG_ERROR |
| R26 | 裁决·分类自写多数投票 | 自洽采样投票不复用 `annotate._majority_vote`（其无过半回退首样本，语义不合） | classify 自写投票（single 多数票、无过半归兜底；multi 逐标签 > n/2、全落选归兜底）；gather 骨架抄 annotate 结构而非 import |
| R27 | 裁决·分类自钉 UI 模板 | UI 模态 Part 组装三处各自为政（模板逐字冻结、标签互异） | classify 抄 `annotate.py:78-88` 单记录形态，在 CONTRACTS §10 钉死自己的逐字模板 |
| R28 | 裁决·估算按全局注记下界 | dry-run 在 per-class quality 覆盖下静态不可精确算 | quality/annotate/verify 估算按全局继承配置；存在 `[class.*]` 覆盖或 multi 时 stderr 注明「按全局配置估算 / multi 按标签乘数 1 报下界」 |
| R29 | 裁决·分类事件仅入 trace | classify 事件的 stderr 级别与默认通道 | `classify.decision` 为 trace-only（无 stderr 镜像，同 quality.judgment）；默认 `trace.channels` 保持 `["quality","verify","schema"]` 不变，用户显式加 `"classify"`；reason 请求条件 = trace.enabled 且 channels 含 classify |
| R30 | 裁决·手册新章追加制 | 手册新章编号：链序插入需全书重排 | **追加制**：新章 `docs/manual/24-classify.md`，README 目录与第 9–13 章交叉引用它（§7 的手册新章编号开放问题，可改） |
...
**内部 Schema**（`schema_engine.classification_schema`，逐字入 CONTRACTS §10.7；关键字集 ⊆ 既有冻结集，**无 uniqueItems**，裁决·Schema 不写 uniqueItems）：
...
| 事件与计数 | 每记录一条 `classify.decision`（payload：`label`、`labels`（multi 携带全集）、`source`、`reason`†、`sc`†）；计数器（M13 属主）：`classify.classes.<name>`（逐标签计）、`classify.fallback`、`classify.failures`、`classify.multi_label_records`；`counts.fanout` 由 M10 计（裁决·fanout 计量归编排器） |
...
Stage 契约（spec §4.3 / `stage.py` docstring / CONTRACTS §5，在既有四条款——仅处理 active 条款、不删元素条款、生成子批例外、失败不逃逸条款——上增补）：

> **多标签扇出例外（classify，仅 `assignment="multi"`）**——可向传入列表**尾部**追加派生信封；追加物视同批内普通元素、同受仅处理 active 条款、生成子批例外与失败不逃逸条款约束；不得删除、重排或替换任何既有元素对象（既有元素的 status / classification / errors 字段写入属仅处理 active 条款与失败不逃逸条款的正常行为）；返回值仍须是传入的同一列表对象（调用方依赖列表身份）。
...
| `classify.llm` | str | "default" | profile 引用；UI 模态须 supports_vision（M1 校验）；计入密钥解析/视觉必需/probe 三处引用集（裁决·三处引用集补分类） |
...
| `classify.sc_temperature` | float | 0.7 | 仅 self_consistency ≥ 3 生效（裁决·补分类采样温度键） |
...
- **选择组**（裁决·选择组合并）：类提供 selection/threshold/top_ratio 任一 ⇒ 合并视图剔除全局侧的互斥对键；互斥校验跑在合并后视图上；
- **rubric**（裁决·类 rubric 合并重解析）：合并 selector → `_resolve_rubric` 重解析；pointwise 6 级校验跑在（类有效 mode × 类有效 rubric）组合上；`[class.X.rubric]` 在场但 selector 非 inline ⇒ 忽略 + warning；
- 类 examples 干跑全局用户 Schema 与 validator（错误定位 `[[class.<name>.annotate.examples]][N]`）；
- `enabled=false` 而 `[[classify.classes]]` / `[class.*]` 在场 ⇒ **warning**（裁决·关开关留配置仅警告，偏离提案的 CONFIG_ERROR）。

配置 dataclass（CONTRACTS §6.1 verbatim；`ResolvedConfig` 增 `classify: ClassifyConfig` 与 `class_views: Mapping[str, ClassView]` 两**必填**字段，裁决·配置全必填风格）：
...
| 算子 | 修订点 |
|---|---|
| M4 quality（spec §3.4.3 增补） | active 项按 `classification.label` 分池（classify 关闭 ⇒ 单一匿名池 = 现行为，零变化回归锚）。两阶段执行（裁决·分池两阶段预抽派发）：同步按类名字典序逐池预抽配对计划（消费 ctx.rng），再跨池合并一个 gather 派发。每池取 `class_views[label]` 的 (QualityConfig, Rubric)：mode/rounds/rubric/threshold/selection/top_ratio 池内生效；judges/both_orders/criteria_per_call/llm/on_unscored 恒为全局。池级 try/except 隔离（裁决·池级失败隔离）。N=1 池沿用现行单条规则。top_ratio 名额基数 = 池内 scored 存活数。计数器与统计升维（裁决·平局计数按池升维）：classify 启用时 `quality.tie_outcomes.<pool>.<crit>`；`quality.bt_fit`/`gate`/`judgment` payload 增 `pool`（裁决·打分事件增 pool 字段）。切口：`_run_pairwise`/`_run_pointwise`/`_set_aggregates`/`_apply_gate` 加 (q, criteria, pool) 参数；`fit_bradley_terry`、`_pairing_plan`、`_percentile_scores`、`_top_ratio_selection`、prompt builder 全部不动。 |
| M5 annotate（spec §3.5.2 增补 + CONTRACTS §7.4 修订，裁决·标注签名增 label 形参） | `build_annotate_prompt(..., label: str \| None = None)`、`annotate_record(..., label: str \| None = None)`——label 非 None 时 instruction/examples 取 `class_views[label].annotate`；stage 层传 `item.classification.label if item.classification else None`。 |
| M6 generate（spec §3.6.2 增补） | 种子按 label 分组（裁决·种子门槛按类默认链）；类段按字典序拼接全局调用序（裁决·类段字典序拼接）；`CallPlan` 增 `class_name`，`one_call` 按类取 instruction/temperature（裁决·生成计划携带类名）；styles 按类抽、llm 仍全局序号轮转/加权；`postprocess_samples` 返回 (Record, class)，构造 PipelineItem 带 `Classification(label, (label,), "inherited", {})`；桶 key classify 启用时为 `<class>×<llm>×<style>`。generate_only：`generate_all` 扁平路径不动（全局指令），产物由链上 classify 正常分类。 |
| M7 verify（spec §3.7.2 增补，裁决·评审任务指令按类取值） | `build_verify_prompt(..., label)`——`[任务指令]` 段与 `extra_criteria` 均取类有效值；`_judge_round`/`_reannotate` 增 label 形参并透传（repair 重标注调 `annotate_record(..., label=...)`）。 |

### 3.5 编排与输出

**M10（spec §3.10.3 增补）**：`_CHAIN_ORDER = ("dedup", "classify", "quality", "generate", "annotate", "verify")`；`_compose_chain` enabled 表加 classify（主链/回流链/generate_only 链均含，继承项靠幂等跳过）；`counts.fanout` = classify 阶段前后 `len(batch)` 差值，M10 计（裁决·fanout 计量归编排器）；`batch.end` payload 增 `fanout`（裁决·批事件增 fanout 字段）；熔断残差公式 `+ fanout`（裁决·残差公式加 fanout 项）；`_estimate` 增 `classify_calls`（裁决·分类调用量估算修正的公式）+ stderr 注记（裁决·估算按全局注记下界）。

**M11（spec §3.11 / §6.3 / §6.4 增补）**：`_meta` 增恒在键 `"classification": {"label", "labels", "source"}`（未启用 = null）；`_meta.scores` 增 `pool`（仅 classify 启用）；rejects `_meta` 增 `label` 键（仅 classify 启用，裁决·rejects 与事件增 label 键——§9.2 五键枚举修订为六键）。report：
...
- `trace.channels` 枚举 7→8（加 `"classify"`；默认值不变，裁决·分类事件仅入 trace）；唯一改点 `loader.py:67 _TRACE_CHANNELS`。
...
§1 包布局表加 `classify.py`；§2 架构 recap 链序；§3 `types.py` verbatim（Classification/PipelineItem）；§4 `errors.py` verbatim（新 kind；顺带补齐存量漂移：`SchemaViolation` 缺 v1.5 `callback_only` 参数的镜像）；§5 Stage 契约多标签扇出例外与「返回同一列表」句；§6.1/§6.3 配置 dataclass 与校验清单；§7 新增 classify API 节 + **§7.4 两签名修订（裁决·标注签名增 label 形参）** + §7.5/§7.9 规范段；§8.1 事件目录；§9.1 `_meta`、§9.2 rejects 六键、§9.3 计数器词表（`counts.fanout`、`classify.*`、tie 计数器池维键式）与所有权表、不变量；§10 新增 §10.8 分类提示词模板 + §10.7 增 `classification_schema`（注明去重在代码侧归一化）；§12 决策登记。
...
| `spec/20-ch2-overall-design.md` | 修改 | §2.1.1 功能表加分类行；§2.1.2 的「不做训练数据配比」负边界补「不做跨类输出配比」划界；§2.2.1 模块表 + 图 2-1/2-2 加 M13；§2.3.1 开关矩阵与约束；§2.3.2 加 classify/扇出行；§2.4 dry-run 行；§2.5 配置总览 |
| `spec/301-m1-config.md` | 修改 | §3.1.4 classify 全量校验清单 + 合并语义（裁决·选择组合并 / 裁决·类 rubric 合并重解析 / 裁决·关开关留配置仅警告 / 裁决·三处引用集补分类 / 裁决·类节白名单外键报错） |
| `spec/304-m4-qualityqurating.md` | 修改 | §3.4.3 分池规格（§3.4 修订条目） |
| `spec/305-m5-annotate.md` | 修改 | §3.5.2 label 形参与类有效取值 |
| `spec/306-m6-generate.md` | 修改 | §3.6.2 类段/继承/桶 key/门槛链（裁决·生成计划携带类名 / 裁决·类段字典序拼接 / 裁决·种子门槛按类默认链） |
| `spec/307-m7-verify.md` | 修改 | §3.7.2 [任务指令] 与 extra_criteria 按类（裁决·评审任务指令按类取值） |
...
| `spec/40-ch4-data-structures.md` | 修改 | §4.1 Classification + PipelineItem；§4.3 契约多标签扇出例外 |
...
| 文件 | 动作 | 内容 |
|---|---|---|
| `24-classify.md` | 新建 | 专章：直觉/配置/两种 assignment/扇出与行唯一键/纯打标模式/调优与 fallback 诊断；样例取自 `examples/classify` 真实运行（编号策略见 §7 的手册新章编号开放问题） |
| `README.md` | 修改 | 目录表加新章行 + 阅读路线 |
| `01` | 修改 | §1.4 算子总览表加 classify 算子行 |
...
| `04-concepts.md` | 修改 | Stage 契约的不删元素条款（铁律）加多标签扇出例外；守恒恒等式加 fanout 项；§4.5 组合约束/菜谱表加 classify |
...
| `19-tutorial-1-minimal.md` | 修改(轻) | §19.5「关闭算子置 null」枚举句补 classification |
...
| 文件 | 改动 |
|---|---|
| `classify.py`（新） | M13 本体：模板组装/自洽采样投票/归一化/fallback/multi 扇出/事件计数（事件名用模块内字面量，仿 quality.py） |
| `config/model.py` | ClassSpec/ClassifyConfig/ClassView；ResolvedConfig 两必填字段（裁决·配置全必填风格） |
| `config/loader.py` | `_parse_classify`；`[class]` raw 透传 + 白名单合并（选择组/rubric 重解析/provenance）；抽 `_resolve_rubric` 与 few-shot 干跑 helper；referenced/vision_users 加 classify；`_TRACE_CHANNELS` 加 "classify"；裁决·关开关留配置仅警告的 warning |
| `types.py` | Classification + PipelineItem.classification |
| `stage.py` | 契约 docstring 增多标签扇出例外 |
...
| 文件 | 要点 |
|---|---|
| `test_classify.py`（新） | Schema 形状（无 uniqueItems）；模板组装确定性（类别表/few-shot/UI 三段）；归一化（去重/兜底剔除/声明序/纯兜底）；自洽采样两模式与弃权；扇出（首标签+克隆追加序/同列表对象/Record 与 dedup 引用共享/容器独立）；幂等跳过；fallback 两形态；非 active 不处理 |
...
| （横切） | 直接构造 ResolvedConfig 的 ~14 个测试文件机械补 classify/class_views 两参（裁决·配置全必填风格） |
...
**验收标准**（在提案 §6 基础上并入审查结论）：① `examples/classify` 真实运行——report.classify 分布合理、trace 抽查各类走各自 instruction、counts 不变量成立；② multi 双意图 fixture 两行 / single 锁定一行、行键 (`_meta.id`, label) 唯一、`counts.fanout` 与扩展不变量（含熔断残差）成立；③ enum 硬约束下不出词表外标签、fallback 可触发且 rejects 归因不被污染（裁决·fallback 留痕不写 errors 回归）；④ `classify.enabled=false` 全量回归逐字段一致（除 `classification: null`）；⑤ `--strict`/dry-run/熔断交付语义不变；⑥ 选择组合并回归（全局 threshold + 类 top_ratio 不误报互斥）。
...
- **已知锐边（记录不修）**：quality 的 `judgment_invalid` 留痕在记录后续失败时会以 `errors[0]` 误归因 rejects（罕见路径，先于本特性存在；classify 侧已用裁决·fallback 留痕不写 errors 规避同型问题）。
- **确定性条件化声明（写入 spec §2.6 幂等行）**：分池构成与扇出以分类输出为条件——temperature 0 下端点无逐字节保证，配对计划的可复现性从「仅依赖 seed」条件化为「以分类结果为条件」（generate 回流已有同类先例）。

## 7. 开放问题（默认裁决随本 spec 生效，需求方可改）

| 议题 | 默认裁决 | 备选 |
|---|---|---|
| 模块编号 | **追加 M13** | — |
| fallback 语义 | **普通类成员、必填** | — |
| generate_only 按类配比 | **不做**，与 §8.3 O6 一并立项 | — |
| 白名单放开 per-class `quality.llm`/`annotate.llm` | **v1 不放** | — |
| 纯打标模式显式开关 | **不加**（零覆盖自然退化） | — |
| 多标签中间档（仅打标不扇出） | **暂不加**，`assignment` 枚举留扩展位 | — |
| dry-run multi 口径 | **乘数 1 报下界 + stderr 注明**（裁决·估算按全局注记下界） | — |
| `enabled=false` + 类配置在场 | **warning**（本 spec 的裁决·关开关留配置仅警告，偏离提案的 CONFIG_ERROR——对齐仓库 no-op 诊断惯例） | — |
| 手册新章编号 | **追加制 `24-classify.md`**（裁决·手册新章追加制） | 链序插入（第 10 章前），代价是全书章号重排与交叉引用重写 |
