...
基于 LLM API 的**无状态批处理数据标注流水线**：把采集数据的去重、质量打分、自动标注，以及（可选）时序流分段、动作摘取、合成生成与二次校验固化成一条可配置的命令行流水线。输入一批 JSONL（或「截图 + UI 控件树」文件对），输出结构由你用 JSON Schema 定义、由代码规则引擎保证合法的 JSONL——每一行都必然通过 Schema 校验，这是机制而非概率。

```
原始数据 ──▶ （可选）时序分段 ──▶ 去重 ──▶ （可选）分类 ──▶ （可选）动作摘取 ──▶ 质量打分 ──▶ 自动标注 ──▶ （可选）二次校验 ──▶ 结构合法的 JSONL
                         │          │                                               │            │                    │
...
- **算子化流水线**：分段 / 去重 / 分类 / 摘取 / 质量 / 标注 / 生成 / 校验八个算子独立开关，编排器只做组合调度（对标 Data-Juicer、distilabel、Dolma 的算子体系）
...
一分半钟后得到主输出（每行 = 你的 Schema 字段 + 可选 `_meta` 履历）、拒绝通道与运行报告。五个示例工程分别覆盖：文本去重+打分+标注（`examples/text`）、UI 截图多模态全流程（`examples/ui`）、从零合成数据集（`examples/generate`）、分类路由与按类打分/标注（`examples/classify`）、时序 UI 流的 episode 分段与动作摘取（`examples/stream`）。

## 文档

| 文档 | 内容 |
|---|---|
| [用户手册](docs/manual/README.md) | 28 章教科书式手册：安装、数据排布、逐参数配置、算子调优、从易到难五篇实战教程 |
| [设计规格](spec/) | 实现级设计规格（v1.4 + v1.5/v1.6/v1.7/v1.8 修订）：每个模块的职责、算法、配置与 IO 契约，每处算法选择均有论文/工业项目背书 |
