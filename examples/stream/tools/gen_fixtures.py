...
生成五个场景子目录共 53 对 uitree_N.jsonl / image_N.png 到
examples/stream/data/（`[stream] key = ["source_dir"]` 按子目录分会话；
index 命名空间全树唯一、各场景错开编号）：

  s1-serial-noise/ 1–14     「串联+噪声」：任务 A 点外卖 8 帧
    （package=com.example.food，帧 5 除外——com.example.social 无关屏，
    预期 dropped_noise）+ 任务 B 打车 5 帧（com.example.taxi）背靠背
    + 帧 14 桌面屏（com.example.launcher，预期 below_min_len 落选后
    以救援候选身份判 new、不并入）。实体延续：餐厅名「川味麻辣烫」跨
    3/4/6，金额 ¥32 跨 4/6/7，订单号仅帧 8；「虹桥机场」跨 10/11/12，
    ¥58 跨 11/12。预期 2 段 2 线索、零缝合零接缝。
  s2-single-cross/ 201–210  「单交叉」：任务 A 点外卖前半 4 帧（201–204，
    ...
    缝成 2 线索（A=双碎片 1 接缝）。
  s3-multi-cross/ 301–315   「多交叉」：任务 A 订酒店 3+3+3 帧（301–303 /
  ...
  s4-noise-rescue/ 401–409  「噪声+救援」（dev-spec §1.1 规范布局
    ...
    1 噪声帧。
  s5-pure-noise/ 501–505    「纯噪声负样本」：锁屏 / 广告弹窗 / 系统更新 /
...
# 每帧 4–10 节点：FrameLayout 根 + TextView 标题（首个非空 text，即
...
# ── s1-serial-noise：点外卖（1–8，帧 5 噪声）+ 打车（9–13）+ 桌面（14） ────
S1_FRAMES: list[tuple[str, list[tuple]]] = [
    ...
    # 14 桌面屏（会话尾部自然帧：预期 below_min_len 落选 → 救援候选判 new）
    ("launcher", [
        ("FrameLayout", "", (0, 0, 400, 800)),
...
# ── s2-single-cross：点外卖（201–204 / 208–210）× 回复消息（205–207） ──────
...
# ── s3-multi-cross：订酒店（301–303/307–309/313–315）× 打车（304–306）
#    × 记备忘（310–312） ─────────────────────────────────────────────────────
S3_FRAMES: list[tuple[str, list[tuple]]] = [
    # 301 酒店首页
    ("hotel", [
        ("FrameLayout", "", (0, 0, 400, 800)),
...
# ── s4-noise-rescue：网购（401–404 + 409 尾段）× 新闻（405–407）
#    + 408 噪声帧（§1.1 规范布局：尾段与其线索尾碎片之间隔着 B 与噪声帧） ────
S4_FRAMES: list[tuple[str, list[tuple]]] = [
    # 401 商城首页
    ("shop", [
        ("FrameLayout", "", (0, 0, 400, 800)),
...
# ── s5-pure-noise：五帧互不相关的插入屏（负样本协议 E2：零线索零缝合） ────
S5_FRAMES: list[tuple[str, list[tuple]]] = [
    # 501 锁屏
    ("lockscreen", [
        ("FrameLayout", "", (0, 0, 400, 800)),
...
SCENARIOS: list[tuple[str, int, list[tuple[str, list[tuple]]]]] = [
    ("s1-serial-noise", 1, S1_FRAMES),
    ("s2-single-cross", 201, S2_FRAMES),
    ("s3-multi-cross", 301, S3_FRAMES),
    ("s4-noise-rescue", 401, S4_FRAMES),
    ("s5-pure-noise", 501, S5_FRAMES),
]
