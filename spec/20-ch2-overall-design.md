...
| 时序流分段与动作摘取 | 可选，默认关 | v1.8（stream 模式，`segment.enabled = true`）：数据按时间排序输入时，摄取层按 `[stream]` 声明排序键、做按分区键单调性校验并按 gap/key/上限规则切候选会话（3.2.8），完整会话作为语义单位，batch_size 只限叶任务分组（3.10.3）；M14 segment 在会话内做滑窗 LLM 边界精化与逐帧噪声剔除，把成员帧收拢为序列记录 episode（3.14）；M15 extract 对相邻帧对推断结构化动作 `{action_type, target, value, description}` 写入 `item.transitions`（仅 UI 模态，3.15）；下游 dedup/classify/quality/annotate/verify 按序列形态适配，内置轨迹 rubric `default:trajectory`（附录 A.3）。噪声帧进 rejects（reason = noise \| below_min_len）；关闭时数据产出与 v1.7 逐字段一致（`_meta.stream: null` 除外）。 |
...
**工具级边界：**① 不训练/托管本地模型；② 不做数据库、缓存、checkpoint、断点续跑或跨运行状态；③ 不做数据采集与数据版本管理；④ 不提供服务化/长驻进程；⑤ 不做人工标注界面；⑥ process 与 flat 不提供全局跨类精确配比；⑦ 除配置声明的 LLM API 外不发起网络请求，无遥测；⑧ sequence 不提供 tier、subsequence、filler、旧 quota/rule/window/time-field DSL、旧 sequence hook 或声明失败后的 instruction-only 降级；⑨ sequence 的 stream 工件可以作为普通 process 输入独立重放，但自动重放评分仍是外部职责。
...
| M16 stitch（v1.9） | 把会话内碎片保守缝合为线索：单调选池 LLM 判定 × 机械先验合取 + 有界二遍复评；被并 episode 壳置 `stitched`、幸存信封 Record 重绑、below_min_len 短段救援翻转（契约 ②c，4.3）；机械标定 `seam_indexes`（3.16）。 | 不重分段（M14）；不摘取动作（M15）；不判重（M3）；不跨会话或进程运行；不做帧多重归属。 | M1, M8, M9 |
...
| M11 输出 | process/flat 不新增淘汰判定；sequence 在 attempt 内组装最终 `SequenceRows`，全部接受后一次准备并提交产品。 | process/flat 按 status 分发 main/rejects/report 并组装 `_meta`；sequence 的最终 item 元数据进入 main/primary rows，replay 只从最终 source rows 派生，`prepare_product` 唯一计算 delivery digest，manifest 最后替换。 | process/flat 依据 status 与 trace；sequence 依据 `SequenceRows`、CrossView、retained-content、report 中同一 delivery digest 与 manifest 摘要。 | process/flat 仍按三通道路由；sequence attempt 不写 rejects，失败不替换正式数据。 |
...
| 1 | `--strict` 违反、报告写失败，或 v1.18 sequence slot/noise slot 耗尽；sequence 耗尽不替换 main/stream/success report/manifest，只写 failed report。 |
...
普通 process 流以完整 Session 串行推进，计算组只影响叶任务派发。上游分段和缝合冻结一次，下游容量重算隔离去重增量与全部可变信封，最后一次尝试才提交。完整文本/树/图片是每段的证据范围；不存在实时增量输入或进程重启恢复。该路径的比较池是完整会话同类序列，物理 batch_size 不改变池或随机计划。
