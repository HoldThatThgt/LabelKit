...
labelkit run      --config <config.toml> --project <project.toml>
                  ...
                  [--console auto|plain|rich]           # v1.10：进度显示面三态（7.7）——auto（默认）按判定链选档；plain 与 v1.9 stderr 逐字节等价；rich 为双区内联实时面板；tool.log_format="jsonl" 时强制 plain 不可覆盖（显式冲突 M1 WARN）
