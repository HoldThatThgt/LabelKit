...
| M6 generate | 以种子（process 模式：过质量门记录；generate_only 模式：配置种子池或无种子条件化，3.6.2）按 llms × styles 组合产出新样本 Record（含 generator 溯源），交 M10 回流。（v1.13）时间流形态：`generate_stream.enabled` 时改产**序列**——蓝图 + 帧实现两类内容调用 + 零 LLM 机械交织，返回直装序列信封与时间流工件行（3.6.5）。 | 仅文本模态；单轮回流不递归；不去重/打分/标注（回流后 M3/M4/M5 职责）；（v1.13）不写盘——工件行交 M11 的工件通道落盘（3.11.2）；不做重放与真值比对。 | M1, M8, M9 |
...
| M6 生成 | 增：N → N + G（G ≤ 调用次数 × `generate.num_per_call`，3.6.2）；生成子批回流 M3 起再过滤（图 2-2），实际净增 ≤ G。v1.13 时间流形态：G = 幸存序列条数（`counts.generated`），单位是**序列**而非单条文本——配额（`Σsequences`）是尝试上限，蓝图/实现失败或被逐帧钩子、序列相似度过滤淘汰的序列直接缺席（不产 failed 记录、不补齐）。 | 引入合成样本、改变输出集的「真实 : 合成」构成比；`generate.llms` / `generate.mixture` / `[[generate.styles]]` 决定合成子集的模型与风格分布（3.6.2）；合成占比过高有 model collapse 风险 [36]——合成样本的统一标记为 `_meta.source.generator ≠ null`（v1.4 起：generate_only 模式下 `generated_from` 恒为空，不可作合成判据），可按其用路径二同法后筛控制比例。 | `_meta.source.generator`（≠ null 即合成，v1.4；键集条件形——恒含 `{"llm","style"}`，v1.14 时间流生成的档位表在场时增第三键 `tier_rank`（档位序数），档位表缺省时维持两键，6.3）；`generated_from` 补充种子溯源（process 模式为种子 id 列表，generate_only 恒为空）；回流后各算子事件照常落点。 | 回流中被淘汰者按所在算子的去向进 rejects。 |
...
| `project.toml` | 工程级单次配置。随一次标注任务变化。 | 输入/输出路径与模态、批大小与 seed、各阶段开关与参数、Rubric（内联或选默认）、任务指令与 few-shot、用户输出 JSON Schema（内联或引用外部文件）；v1.7 增分类类别表与按类覆盖（`[classify]` / `[class.<name>.*]` 白名单，5.2）；v1.8 增时序流三节——`[stream]` 输入侧声明（排序与会话规则）、`[segment]`（M14 分段）、`[extract]`（M15 摘取），另有 `annotate.sequence_frames`（5.2）；v1.9 增 `[stitch]`（M16 线索缝合，11 键，5.2）；v1.12 增帧粒度三族——`[frame.classify]`、`[frame.annotate]` 与 `[frame.class.<name>.annotate]`（5.2）；v1.13 增 `[generate.stream]`（时间流生成形态，7 键）与两族按类重资产——`[class.<name>.annotate].schema_*`（按序列类标注 Schema）与 `[frame.class.<name>.generate]`（帧类内容契约），`[stream]` 在本形态下改作生成侧铺设契约（5.2）。见 5.2。 |
