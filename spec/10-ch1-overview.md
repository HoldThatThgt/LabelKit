...
| （v1.10 评审补充）Console 实时面板（TUI）：交互终端下以双区内联面板实时呈现批进度、流水线段棋盘、状态账与 LLM 用量/密钥池/熔断（工业调研裁决品类：批处理工具不做全屏 TUI）；非 TTY / CI / jsonl 下逐字节保持 v1.9 行为 | 7.7 三态 console 规格；`[console]` 配置 5.1 与 `--console`（2.4）；ProgressListener 旁路（3.12.3）；依赖面增 rich（2.6）；裁决 U1–U18 见 `docs/dev/SPEC-tui-console.md` §2 |
...
**Console 实时面板（v1.10 对齐，2026-07-17）**：需求为「deep-search 各种工业项目，为 LabelKit 设计一个 TUI 方案」；跨四品类约 20 个工业项目的调研（buck2 superconsole / Docker BuildKit / bazel / cargo·uv·pip / Nextflow·Snakemake / k9s·htop / tqdm / Bespoke Curator·distilabel / Claude Code·Codex CLI，引用 [C-1]–[C-20] 见 `docs/dev/PROPOSAL-tui-console.md`）收敛为**十八项设计裁决 U1–U18，详表见 `docs/dev/SPEC-tui-console.md` §2**。需求方 2026-07-17 裁决：**规格定稿、实现另行排期（spec-only）**；U4 批准 `rich` 入 §2.6 依赖白名单；U18 批准对 v1.9 T16「进度行固定键集」的**有界修订**（rich 面板状态账展示 stitched/threads 与 report.counts 口径对齐；plain 进度行与文本版终版摘要键集逐字节不动——T16 约束收窄为 plain 面专属）；U14 心跳默认 0（关，保逐字节回归锚）；U15 键盘交互**一期实施**（推翻提案暂缓推荐；封闭键集 + cbreak 非全 raw——Ctrl-C 语义不变，`console.interactive=false` 一键回避）。核心裁决择要：品类 = 双区内联面板、永不进 alternate screen（U1，批任务保 scrollback）；面板 = M12 第四纯消费面，`ProgressListener` 进程内旁路不产生 TraceEvent、不入 7.2 目录（U2/U11——7.2 只增不改原则零触碰）；信息纪律 = stderr 镜像同级，无数据内容/LLM 自由文本/record id（U6 红线）；渲染异常自吞降级 plain、永不影响退出码与产出（U7 红线）；三态 auto|rich|plain，jsonl 强制 plain 不可覆盖，plain 与 v1.9 逐字节等价为回归锚（U5）；批总数分母 UI 模态恒显示、文本模态默认不做估算扫描（U17——不打破 M10「避免双倍输入读」现状，`console.estimate` 显式换购）。
