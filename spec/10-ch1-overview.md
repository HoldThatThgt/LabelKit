...
| 场景规划与时间布局（v1.17） | QuotaCompiler（class × local date 整数计数、具名 quota bucket equality）→ ScenarioPlanner：owner permutation `AddInverse` / `AddElement`、1µs always-present interval `AddNoOverlap` 时间戳唯一、聚合 noise reserve（O(S) 编码）、三层字典序目标逐层冻结、具名 assumption core 诊断（3.6.5） | OR-Tools CP-SAT Python API [118]——取用 `AddInverse`、`AddElement`、interval、`AddNoOverlap`、assumptions 与 model stats 的组合使用面；OR-Tools Job Shop [119]——取用 precedence、resource no-overlap 与 makespan 的成熟分层；Timefold score levels [120]——取用 hard/soft 字典序与拒绝 score folding（逐层冻结的字典序目标先例） |
| 静态校验、路径与 hook 加载（v1.17） | secret-free 静态校验（无 `--probe` 的 validate/dry-run 不读任何 key value）、project-root 相对路径解析（`ResolvedPaths` 恒绝对化）、`path.py:function` 文件路径 hook 加载（`spec_from_file_location`） | Terraform validate [121]——取用「静态一致性检查不访问远端服务」；Ruff configuration [122]——取用「配置内相对路径相对 project root」；Python importlib [123]——取用 `spec_from_file_location` 的文件路径模块加载面 |
| 有限日程与结构化合成工程形态（v1.17） | schedule 必填 start/end 有限化（删除隐式 horizon）、day/week/schedule 三周期 quota、structured noise 复用帧类 instruction 与 JSON Schema、target/delivered/attempts 分立记账（5.2.2） | RFC 5545 [124]——取用「recurrence 必须由 end/count 有限化、时间类型一致」；Hypothesis settings [125]——取用 satisfying target、discard 与 bounded generation 的分离观测；Synthea [126]——取用 module/schema 驱动的结构化合成记录；Simod [127]——取用配置驱动过程模拟与 configuration-relative path（论文本体见 [101]） |
...
**场景规划与精确交付（v1.17 对齐，2026-08-21）**：需求原型为时间流生成工程的大规模场景规划与精确交付——静态规划要在 167 条序列规模完成、日期与配额语义要精确、quota 要表达**成功交付数量**而非尝试次数。处置 E2E-42~60；设计冻结于 `docs/dev/SPEC-scenario-planning.md`（2026-08-21 三路审计修订定稿），**主规格先行同步、实现随后**；沿用自然语言命名制。明确不做清单见 8.1 v1.17 块。逐条择要：

- 裁决·零交叉零变量（E2E-43）——`crossed_sessions = 0` 时 crossing builder 不调用，model stats 中 crossing variables/constraints 必须为 0；owner 映射用 `AddInverse` permutation 而非 `D × S` bool 矩阵，secondary 映射规模 O(S)（167 序列 / 180 task frame 模型低于 capacity，E2E-42）。
- 裁决·三分错误面（E2E-44）——planner capacity / infeasible / budget 是三个不同错误面：capacity 绝不显示为 INFEASIBLE；exit 分流 4 / 2 / 4（7.6）。
- 裁决·有限 schedule 为时间硬边界（E2E-45/53）——`[generate.stream.schedule]` 必填 start/end（半开区间）+ `exclude_dates`（区间外条目 fail-fast）；删除 `_horizon` 每 session 一周递推；一天配置只能生成这一天内的 occurrence。
- 裁决·ScenarioPlan 单次编译（E2E-46）——M1 只调 `compile_scenario` 一次产出冻结 `ScenarioPlan`（`ResolvedConfig.scenario_plan`，非 time-stream 形态为 None），validate/dry-run/run/estimate 共用同一对象并回显同一 `plan_digest`，不重复规划、不漂移目标。
- 裁决·双模型分工——QuotaCompiler（class × local date 非负整数 occurrence count、具名 quota bucket equality、单目标最小化 occurrence 总数、OPTIMAL 解码逐类 target）先冻结 target，ScenarioPlanner 再在有限 slot 上求解时间布局（重新建立同一组 bucket equality，让 frame window、sequence rule 与 resource 共同决定日期）。
- 裁决·length 冻结进 slot——length 在 slot 构建时冻结为 `SequenceSlotSpec.length_target`（`length_range` 只是抽取域与诊断输入），planner 不再改变 active length；tier 构成、frame class word、日期与时间才由 planner 决定；quota 与 tier 分配本身均不消费 RNG。
- 裁决·三层字典序目标——preference deviation（高）→ calendar days spanned（中）→ timeline end（低），整数计算、每层独立 solve 必须 OPTIMAL、冻结等式后再进下一层；solver 固定单线程、`random_seed = run.seed & 0x7fffffff`、每层 `max_deterministic_time = 10.0`、CP-SAT 自动搜索。
- 裁决·聚合 noise reserve——每 session 只有一个 `noise_count` IntVar，编码固定 O(S)（相邻 session 顺序约束保证墙钟区间两两不交 ⇒ `occupied_task_points[s] = max(0, task_count[s] − 2)`）；**禁止逐 (session, frame) 的 reified 计数**——该形态在 167 序列规模约 30 万 entries，单项击穿容量上限。
- 裁决·破坏性配置修订——十项删除键定向 CONFIG_ERROR（不读旧值、不转换、无别名，5.2.2）；hook 引用统一 `path.py:function`（相对 project root，`spec_from_file_location` 加载，不改 `sys.path`）。
- 裁决·quota 双形态（E2E-55/56）——exact counts 与 integer weights（weights 先除以最大公约数，minimum exact cohort = 最简整数比之和；`allocation = "exact"` 要求 total 是 cohort 整数倍，否则同轮给归一化权重、cohort 与上下最近可精确 total；`largest_remainder` 复用纯整数最大余额、平票按表内声明序）；多张 quota 约束同一批 occurrence 不相加；「逐日覆盖」与「长期比例」数学兼容时同时成立，冲突时 core 同时点名两张 quota。
- 裁决·stable slot 与 exact delivery（E2E-60）——quota 是成功交付目标：content 失败只重试该 slot（完整重跑 brief + realization + validators，frame word / timestamps / session / quota / noise slot 不变；accepted payload 不回滚），`max_attempts_per_slot` 有界；耗尽继续其余 slot、原子交付已成功部分、exit 1（7.6）；`delivery.delivered_sequences == counts.generated`，下游 quality/annotate/verify 可使 emitted 更低但不反向触发 M6 refill。
- 裁决·13 桶失败闭集——只记第一个失败阶段，为零也全部在场；守恒 `attempts = delivered_sequences + delivered_noise + sum(failures.values())`（完整非 fatal attempt）。
- 裁决·incremental scenario validator（E2E-57）——`scenario_validator` 增量校验 accepted 前缀 + candidate（`ScenarioValidationInput`），非空违规只拒绝 candidate 重试同一 slot，accepted 不回滚不重排；候选过滤顺序冻结：schema guarantee → `sample_validator` → correlation/temporal replay → `sequence_validator` → similarity filter probe → scenario_validator → commit（similarity probe 与 commit 分离，scenario violation 不污染 similarity state）。
- 裁决·frame duration 与 resource（E2E-58/59）——`duration_s` 闭区间（`Decimal` 微秒量化）、frame rule `contains` 严格包含（`source.start < target.start ∧ target.end < source.end`）、跨序列 resource `AddNoOverlap`（全局互斥、不同 resource 可重叠）；duration 与 length 同用 seeded preference 机制、planner 最小化绝对 deviation；声明 duration 的帧类必须绑定 `end_ts` 或 `duration_s`（不允许 artifact 观察不到的隐藏区间）；时间词表扩 `end_ts` / `duration_s` 两词。
- 裁决·跨序列 sequence rules（E2E-57）——四模板（precedence / response / succession / not_co_existence）按 period bucket 独立执行，occurrence 归属 `sequence_start` 的 local date；v1.16 `SequenceRuleSpec` 更名 `FrameRuleSpec`，名字回收给跨序列规则（无继承关系）；rule / window 各增必填自然名称，四类 name 共处全局唯一域。
- 裁决·structured noise（E2E-54）——`[[generate.stream.noise]]` 帧类 + 权重表复用帧类 generate instruction 与 JSON Schema（Schema 解析、预算检查与 realization 复用 task frame 路径）；truth 保留实际 frame_class；noise 仍是 point occurrence（不得声明 duration / resources、不入生效 tier / rule / window）。
- 裁决·project root 与路径（E2E-47/52）——`project_root = Path(project_path).resolve().parent` 冻结；project TOML 相对路径（input/output、四类 schema_path、trace、四 hook）一律相对 project root，CLI `--input`/`--output` 按 cwd；`ResolvedPaths` 恒绝对化、`report.run.paths` 恒在场；live / dry-run report 文件名固定（`<output-stem>.report.json` / `<output-stem>.dryrun.report.json`，由 M1 写入，emitter 不追加后缀）。
- 裁决·secret-free 静态校验（E2E-50）——删 profile `api_key` 字段（只保存环境变量名）；无 `--probe` 的 validate 与 dry-run 不调用任何环境变量 value reader（monkeypatch 断言零调用）、不发 missing-key WARN；run 与 `validate --probe` 聚合解析全部 referenced profile 的 key value（任一缺失 exit 2）；`RuntimeCredentials` 不入 `ResolvedConfig`、不进 repr / 日志 / trace / report / exception / deepcopy；LLMClient 构造必收之，删除内部 env fallback。
- 裁决·dry-run estimate 入 report（E2E-51）——dry-run 顶层 `estimate` 直接引用 `estimate_run` 返回对象（不复制重算），console 与 JSON 逐键相等；`--limit` 在 time-stream 拒收。
- 裁决·一次性派生检查（E2E-49）——`derive_stream_bounds` 纯函数返回全部错误不短路：length / span / gap 关联错误同轮出现；跨度检查用实际 slot length domain 与 crossed count，不用 `session_max_len` 反推另一错误（零交叉时只要求单序列真实长度与 noise reserve，`session_max_len = max(required min length)` 通过，不要求 `2 × len_max`）。
- 裁决·RNG 五流分立——`scenario.preference` / `scenario.noise` / `delivery.profile` / `delivery.content` / `artifact.duplicate` 五个具名互不借位随机流（各 `Random(f"{seed}:<name>")`）；某 slot 多一次 content retry 不改变其他 slot 的 length、timestamp、noise、profile 或 duplicate 抽签；slot retry 随机流再由稳定 slot key 与 attempt count 派生。
- 裁决·planner 归属与旧栈删除——新 canonical 包 `labelkit/common/runtime/scenario/`（model / quota / calendar / rules / planner / sessions / diagnostics / noise）+ `labelkit/common/runtime/credentials.py`；删除 v1.16 `sequence_planner.py` / `declare.py` / `temporal.py` 与 `orchestration/profile_usage.py`，不留 re-export（3.0.1）。
