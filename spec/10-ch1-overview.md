...
| 交付槽（Delivery slot） | 一个 counterfactual set 与 scenario_index 的精确提交单位；整组先经 generation/evaluation，再经 prospective dedup、pointwise quality、annotate、verify 和双视图核对，成功后一次 group commit。 |
...
| 原子准入与 prospective dedup（v1.18） | counterfactual set 整组经只读 group_probe 与 attempt-local 下游，所有 variant 均通过才在无 await 临界区 group_commit；失败不污染索引或 dataset counters（3.3、3.10） | 数据库式 prepare/commit 与 capability token 是单进程原子准入的成熟形态；LLM usage 作为运行事实不回滚 |
...
- instruction-only 的 planner 在 LLM 前冻结 slot、长度、每个位置的逻辑时间、投影时间与 session。LLM 只能在已声明闭集内选择 frame class、seed actor、意图与 patch；不接收 pattern、variant、expected truth、outcome 或 role 权限。该模式没有 binding，不做 crossing/duplicate，role 机械为 `position_NNN`；root/pre-state/publish/observer 检查跳过，但 patch 闭集/test 顺序/原子执行、基础 state Schema 与可选 hook 仍全部执行，actor knowledge 固定为 semantic 判定。
- timeline 使用 `timestamp_start`、`event_gap_s`、精确 primary/crossed session 数、session 容量/跨度/间隔、noise event 数与 whole-sequence duplicate 数。declared 相邻时间只由 pattern gap/max-span 决定；event_gap 仅用于 instruction-only、noise/replay。duplicate 只重放已交付 positive primary，逐位内容/类/实际 role/顺序同源而 ID/timestamp 新生。
- `compile_scenario_plan(program, seed)` 是唯一 planner 入口。block 最多 4096 primary events；OR-Tools 固定版本、单 worker、固定 seed、每优化层 `max_deterministic_time=10.0`。只解码 OPTIMAL；INFEASIBLE 为配置错误 exit 2，FEASIBLE/UNKNOWN 为 plan budget exit 4，MODEL_INVALID 为 internal exit 4；禁止 greedy fallback。
- 一个 counterfactual set 是一个 AttemptTransaction，其 `items` 是唯一可变 item 真值。slot 与 variant 按声明序串行准入；整组依次 prospective dedup、pointwise quality、annotate、verify、M11 从最终 item 装配 `SequenceRows`、从最终 source 行投影 replay、CrossViewReconciler、retained-content prospective check，最后 group commit。失败丢弃索引/dataset counter delta 并重试整 slot；LLM usage/token/provider retry/latency、SchemaEngine resolved-at 与 trace 事实不回滚。provider fatal、熔断、KeyboardInterrupt、CancelledError 原样穿透且不消耗 attempt。
- group_probe 不改 exact/MinHash/semantic 索引，返回绑定 index generation 与 record digests 的单次 token；group_commit 先验证未消费、generation 与 digest 未变，再在无 await 临界区写入全部索引。replay rows 在 source positive commit 前只从 M11 最终 `SequenceRows.primary_stream_rows` 预构造并与当前 set 的 SequenceRows 一起计入 retained-content 预算，不能从 pre-downstream Record 或 commit 后补造未收费内容。
