...
| 有序异步执行（v1.19） | ResourceKey 独立有界接纳；纯叶任务乱序完成、按输入序归并；stitch 跨会话当前候选组成 wave；declared baseline 后 sibling suffix 结构化并发；sequence 使用连续候选缓冲和声明序短 commit（3.10、3.17） | Python TaskGroup 结构化并发、Apache Flink ordered async I/O 与 Ray pending-task backpressure |
