...
v1.9（线索缝合域）明确不做六条：① **真并发**（同屏双任务/分屏）——单前台屏无真并发 [65]，帧单一归属是手术/归因/守恒的公共地基（帧多标签先例 [72] 经评估否决）；② **跨会话/跨运行缝合**——线索作用域不跨 session 或进程运行；同 session 可跨计算组（3.16.4），无持久化红线不变（2.6）；③ **帧级区间树与子任务嵌套校验**——episode 内子任务跨度经需求方 2026-07-16 裁决不做引擎特性（标注层模式：用户 Schema 自声明 `subtasks: [{label, step_range}]`，工具不校验其语义）；④ **自动拆线手术**——verify 对错缝只标 `wrong_stitch` 不重构（3.7.3）；⑤ **在线/增量处理**——批处理工具定位不变；⑥ **完成感知封闭**（收尾动作模式触发线索封闭）——链序上 extract 后置，缝合运行时无动作证据、「机械收尾动作模式」不可判而撤除（3.16.4 ①）；若未来「extract 先行」次序演进（8.4 M14 行候选）使动作证据先行，可重评（8.4 M16 行演进候选）。
...
| 上下文预算 | 完整请求统一启发式预检与真实端点超限识别；process stream 完整会话、全证据、主动容量分区、不可重开边界及有限重算；必要相邻帧数为 2 | 实际使用的流 profile 必须声明正 context_window；既有固定图片尺寸；普通非流处理保留其合法配置行为 | 输出预算、模型官方计数 API 与文本密度校准须由新的明确需求定义；不在本特性延期项内。 |
